    ...
    <nav className="flex items-center gap-2 px-6 py-4 overflow-x-auto no-scrollbar animate-in fade-in slide-in-from-left-2 duration-500">
      ...
      {breadcrumbs.slice(1).map((crumb, idx) => (
        <div key={crumb.href} className="flex items-center gap-2 shrink-0">
          ...
          {crumb.isLast ? (
            <span className="text-[9px] font-black tracking-wide text-[var(--text-primary)] px-3 py-1.5 rounded-lg bg-[var(--accent)]/5 border border-[var(--accent)]/10">
              {crumb.label}
            </span>
          ) : (
            <Link 
              href={crumb.href}
              className="text-[9px] font-black tracking-wide text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
            >
              {crumb.label}
            </Link>
          )}
