    ...
    <div className="space-y-16 animate-in fade-in duration-700">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <p className="text-[11px] font-black tracking-[0.3em] text-[var(--accent)]  opacity-80">Operational Management</p>
        ...
        <button 
          ...
          className="bg-[var(--text-primary)] text-[var(--bg-primary)] px-10 py-4 rounded-2xl font-black text-xs tracking-wide hover:bg-[var(--accent)] hover:text-white transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95"
        >
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        >
          ...
          <span className="text-[10px] font-black tracking-[0.25em] text-[var(--text-primary)]  opacity-60 group-hover:opacity-100 transition-opacity">New Story</span>
        </div>
            ...
            <div key={status._id} className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden group shadow-[0_20px_50px_-20px_rgba(0,0,0,0.1)] bg-[var(--bg-primary)] border border-[var(--glass-border)]">
              ...
              <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end text-white z-10">
                <div className="space-y-1.5">
                  <p className="text-[10px] font-bold tracking-wide opacity-60">Engagement</p>
                ...
                <div className="flex flex-col items-end gap-2">
                   <div className="bg-white/10 backdrop-blur-xl border border-white/10 px-4 py-1.5 rounded-full text-[10px] font-black tracking-tight  flex items-center gap-2 shadow-xl">
                   ...
                   <div className="bg-white/10 backdrop-blur-xl border border-white/10 px-3 py-1 rounded-full text-[9px] font-black tracking-widest  flex items-center gap-1.5">
        ...
        <div className="space-y-5">
          {statuses.filter(s => new Date(s.expires_at) <= new Date()).length === 0 ? (
            <div className="py-12 text-center bg-[var(--bg-secondary)]/30 rounded-[2rem] border border-dashed border-[var(--glass-border)]">
              <p className="text-xs font-bold text-[var(--text-secondary)] opacity-50 tracking-wide">No archival data available</p>
            </div>
          ) : (
            statuses.filter(s => new Date(s.expires_at) <= new Date()).map(status => (
              <div key={status._id} className="bg-[var(--bg-primary)] p-5 rounded-[2rem] border border-[var(--glass-border)] flex items-center justify-between shadow-sm hover:shadow-xl transition-all duration-500 group">
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-1.5">
                    <p className="font-black text-base text-[var(--text-primary)] tracking-tight  leading-tight">
                ...
                <div className="flex gap-12 items-center pr-4">
                  <div className="text-right">
                    <p className="text-[9px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-1 opacity-40">Views</p>
                    <p className="text-lg font-black tabular-nums tracking-tighter">{status.views_count || 0}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-1 opacity-40">Likes</p>
                    <p className="text-lg font-black tabular-nums tracking-tighter text-red-500">{status.likes_count || 0}</p>
                  </div>
                  <div className="text-right hidden sm:block">
                    <p className="text-[9px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-1 opacity-40">Lifespan</p>
