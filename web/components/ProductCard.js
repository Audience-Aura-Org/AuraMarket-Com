          ...
          <div className="flex-1 flex flex-col justify-between py-1 min-w-0">
            <div className="space-y-2">
               <div className="flex items-center justify-between gap-4">
                  <Link href={`/shop?vendorId=${vendorId}`} className="flex items-center gap-2 min-w-0" onClick={e => e.stopPropagation()}>
                    ...
                    <span className="text-[11px] lg:text-[12px] font-bold text-[var(--accent)] tracking-normal truncate whitespace-nowrap block flex-1">{vendor_id?.store_name || 'Verified vendor'}</span>
                  </Link>
                  {user?._id !== vendorUserId && (
                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleFollow(); }} disabled={followLoading} className={`text-[11px] lg:text-[12px] font-bold tracking-tight ${isFollowing ? 'text-emerald-500' : 'text-[var(--text-secondary)] hover:text-[var(--accent)]'}`}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-70">
            ...
            <div className="flex items-center gap-3 mt-auto">
              <button 
                ...
                className="h-9 px-6 bg-[var(--text-primary)] text-[var(--bg-primary)] text-[11px] lg:text-[12px] font-bold tracking-tight rounded-2xl flex items-center justify-center hover:bg-[var(--accent)] hover:text-white transition-all shadow-md active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed"
              >
      ...
      >
        <div className="p-2 sm:p-2.5 md:p-3 flex items-center justify-between gap-1 sm:gap-2 border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/50 backdrop-blur-md">
           <Link href={`/stores/${vendorId}`} className="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden group/vendor" onClick={e => e.stopPropagation()}>
              ...
              <h4 className="text-[11px] lg:text-[12px] sm:text-[12px] font-bold text-[var(--text-primary)] leading-none flex-1 truncate">{vendor_id?.store_name || 'Verified node'}</h4>
              {vendor_id?.verified && <Check className="size-2.5 text-blue-500 shrink-0" />}
           </Link>
           
           {user?._id !== vendorUserId && (
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleFollow(); }} disabled={followLoading} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-full text-[10px] lg:text-[12px] md:text-[11px] lg:text-[12px] font-bold tracking-tight transition-all active:scale-95 shadow-sm border ${isFollowing ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-[var(--accent)] text-white border-[var(--accent)] hover:brightness-110'}`}>
                 {isFollowing ? 'Following' : '+ Follow'}
              </button>
           )}
        </div>
        ...
        <div className="relative aspect-square overflow-hidden bg-[var(--accent)]/5">
          ...
          {!inStock && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-10">
              <span className="px-4 py-2 bg-red-500 text-white text-[11px] lg:text-[12px] font-bold tracking-tight rounded-full shadow-xl">Out of stock</span>
            </div>
          )}
          ...
          <Link href={`/shop?vendorId=${vendorId}`} onClick={e => e.stopPropagation()} className="absolute bottom-3 left-1/2 -translate-x-1/2 w-[90%] bg-white/10 backdrop-blur-2xl border border-white/20 rounded-xl py-2 flex items-center justify-center gap-2 text-white text-[11px] lg:text-[12px] font-bold tracking-tight transform translate-y-3 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 z-20">
        ...
        <div className="p-2 sm:p-2.5 md:p-3.5 flex flex-col flex-1 gap-2 md:gap-3">
          <div className="space-y-0.5 md:space-y-1">
            <Link href={`/products/${productId}`} className="block">
              <h3 className="text-[11px] lg:text-[12px] sm:text-[12px] md:text-[14px] font-bold text-[var(--text-primary)] truncate group-hover:text-[var(--accent)] transition-colors tracking-tight">{name}</h3>
            </Link>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 opacity-70">
                 ...
                 <span className="text-[10px] lg:text-[12px] sm:text-[11px] lg:text-[12px] md:text-[12px] font-bold text-[var(--text-secondary)]">{rating || '4.8'}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-1 md:gap-1.5 mt-auto">
            <button 
              onClick={handleBuyNow} 
              disabled={!inStock}
              className="flex-1 h-8 md:h-9 bg-[var(--text-primary)] text-[var(--bg-primary)] text-[10px] lg:text-[12px] md:text-[11px] lg:text-[12px] font-bold tracking-tight rounded-lg md:rounded-xl hover:bg-[var(--accent)] hover:text-white transition-all active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed"
            >
