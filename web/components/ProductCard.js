            ...
            <div className="space-y-2">
               <div className="flex items-center justify-between gap-4">
                  <Link href={`/shop?vendorId=${vendorId}`} className="flex items-center gap-2 min-w-0" onClick={e => e.stopPropagation()}>
                    ...
                    <span className="text-[10px] font-bold text-[var(--accent)] tracking-normal truncate whitespace-nowrap block flex-1">{vendor_id?.store_name || 'Verified Vendor'}</span>
                  </Link>
    ...
    return (
      ...
      >
        <div className="p-2 sm:p-2.5 md:p-3 flex items-center justify-between gap-1 sm:gap-2 border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/50 backdrop-blur-md">
           ...
           {user?._id !== vendorUserId && (
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleFollow(); }} disabled={followLoading} className={`px-2 md:px-3 py-1 md:py-1.5 rounded-full text-[7px] md:text-[8px] font-bold tracking-tight transition-all active:scale-95 shadow-sm border ${isFollowing ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-[var(--accent)] text-white border-[var(--accent)] hover:brightness-110'}`}>
                 {isFollowing ? 'Following' : '+ Follow'}
              </button>
           )}
        </div>
