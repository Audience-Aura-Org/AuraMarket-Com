  ...
  if (!shipments.length) {
    return (
      <div className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-10 text-center">
        <p className="text-[11px] font-bold tracking-tight opacity-40">No shipments assigned yet</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] text-left">
          <thead className="border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/50">
            <tr className="text-[11px] font-bold  tracking-[0.2em] text-[var(--text-secondary)]">
            ...
            {shipments.map((s) => (
              <tr key={s._id} className="hover:bg-[var(--accent)]/5">
                <td className="px-5 py-4">
                  <p className="text-xs font-bold tracking-tight">{s.tracking_code}</p>
                  <p className="text-[11px] font-bold  opacity-60">
                    Order: #{(s.order_id?._id || s.order_id || "").toString().slice(-8).toUpperCase()}
                  </p>
                  <p className="text-[11px] font-bold opacity-50">{new Date(s.createdAt).toLocaleString()}</p>
                </td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4 text-xs font-bold">{(s.price || 0).toLocaleString()} XAF</td>
                <td className="px-5 py-4">
                  <span className="rounded-full border border-[var(--glass-border)] bg-[var(--bg-secondary)] px-2 py-1 text-[11px] font-bold ">
                ...
                <td className="px-5 py-4 text-right">
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg border border-[var(--glass-border)] px-3 py-2 text-[11px] font-bold "
                  >
