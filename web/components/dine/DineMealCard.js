...
export default function DineMealCard({ meal, onSelect = null }) {
  const { vendor_id, name, price, thumbnail_url, rating, restaurant_name, restaurant_logo_url, category_id, prep_time_minutes, restaurant_open, vendor_subscription_active } = meal;
  ...
  const subscriptionInactive = vendor_subscription_active === false;
  const mealUnavailable = restaurantClosed || subscriptionInactive;
  ...
  return (
    <div className={`group relative rounded-[2rem] bg-[var(--glass-bg)] border border-[var(--glass-border)] shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden hover:-translate-y-1.5 backdrop-blur-xl flex flex-col h-full font-poppins${mealUnavailable ? ' opacity-60' : ''}`}>
        ...
        {mealUnavailable && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10">
            <span className="text-[11px] font-bold text-white px-3 py-1 bg-black/60 rounded-full">
              {restaurantClosed ? 'Closed' : 'Unavailable'}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="p-2 sm:p-2.5 md:p-3.5 flex flex-col flex-1 gap-2 md:gap-3">
        ...
        <div className="grid grid-cols-3 items-center gap-1 md:gap-1.5 mt-auto">
          <button
            ...
            disabled={mealUnavailable || (!onSelect && adding)}
            title="Add to cart"
