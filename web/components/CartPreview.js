...
export default function CartPreview() {
  ...
  return (
    <div className="absolute right-0 top-full mt-4 w-80 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-[2rem] shadow-2xl p-4 transition-all duration-300 transform-gpu translate-y-2 z-50 md:opacity-0 md:pointer-events-none md:group-hover:opacity-100 md:group-hover:pointer-events-auto md:group-hover:translate-y-0 backdrop-blur-xl">
      <h4 className="text-[10px] font-black  tracking-[0.2em] mb-4 text-[var(--text-secondary)]">Cart Preview</h4>
      {loading ? (
        ...
        <div className="py-10 text-center text-[var(--text-secondary)]">
          <div className="text-xs font-bold opacity-40 tracking-wide">Inventory Empty</div>
        </div>
      ) : (
        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
          {items.slice(0,8).map(it => (
            <div key={it.id || it.productId} className="flex items-center gap-3 group/item">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-black truncate text-[var(--text-primary)] tracking-tight">{it.name}</div>
          ...
          <div className="pt-4 border-t border-[var(--glass-border)] mt-4 flex items-center justify-between">
            <div className="text-[10px] text-[var(--text-secondary)] font-black tracking-wide">Subtotal</div>
            <div className="font-black text-sm tracking-tighter">{subtotal.toLocaleString()} XAF</div>
          </div>
          <div className="mt-4 flex gap-2">
            <Link href="/cart" className="flex-1 px-4 py-3 rounded-full bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[var(--text-primary)] font-black text-[10px] text-center tracking-wide hover:border-[var(--text-primary)] transition-all">View Cart</Link>
            <Link href="/checkout" className="px-5 py-3 bg-[var(--accent)] text-white rounded-full font-black text-[10px] flex items-center gap-2 tracking-wide shadow-lg shadow-[var(--accent)]/20 hover:scale-105 transition-all"><ArrowRight className="w-3.5 h-3.5" />Checkout</Link>
          </div>
        </div>
      )}
    </div>
