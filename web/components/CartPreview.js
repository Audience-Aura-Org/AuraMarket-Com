...
export default function CartPreview() {
  ...
  return (
    <div className="absolute right-0 top-full mt-4 w-80 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-[2rem] shadow-2xl p-4 transition-all duration-300 transform-gpu translate-y-2 z-50 md:opacity-0 md:pointer-events-none md:group-hover:opacity-100 md:group-hover:pointer-events-auto md:group-hover:translate-y-0 backdrop-blur-xl">
      <h4 className="text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] mb-4 text-[var(--text-secondary)]">Cart Preview</h4>
      {loading ? (
        ...
        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
          {items.slice(0,8).map(it => (
            <div key={it.id || it.productId} className="flex items-center gap-3 group/item">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] lg:text-[12px] text-[var(--text-secondary)]  font-semibold mt-0.5 opacity-60 ">{it.quantity} × {it.price.toLocaleString()} XAF</div>
              </div>
          ...
          <div className="pt-4 border-t border-[var(--glass-border)] mt-4 flex items-center justify-between">
            <div className="text-[10px] lg:text-[12px] text-[var(--text-secondary)]  font-semibold tracking-tight">Subtotal</div>
            <div className=" font-bold text-sm tracking-tighter">{subtotal.toLocaleString()} XAF</div>
          </div>
          <div className="mt-4 flex gap-2">
            <Link href="/cart" className="flex-1 px-4 py-3 rounded-full bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[var(--text-primary)]  font-semibold text-[10px] lg:text-[12px] text-center tracking-tight hover:border-[var(--text-primary)] transition-all">View Cart</Link>
            <Link href="/checkout" className="px-5 py-3 bg-[var(--accent)] text-white rounded-full  font-semibold text-[10px] lg:text-[12px] flex items-center gap-2 tracking-tight shadow-lg shadow-[var(--accent)]/20 hover:scale-105 transition-all"><ArrowRight className="w-3.5 h-3.5" />Checkout</Link>
          </div>
        </div>
      )}
    </div>
