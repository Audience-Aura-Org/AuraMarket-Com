  ...
  const generateSKUs = (types) => {
    ...
    const helper = (depth, current) => {
      if (depth === validTypes.length) {
        // Try to find existing SKU match to preserve price/stock
        const existingMatch = skuVariants.find(sv => 
          Object.entries(current).every(([k, v]) => sv.combination[k] === v)
        );
        combs.push({ 
          combination: { ...current }, 
          price: existingMatch?.price || form.price || 0, 
          stock: existingMatch?.stock || form.stock || 0 
        });
        return;
      }
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10 w-full">
        ...
        <header className="h-20 flex items-center justify-between px-10 glass-panel border-b border-[var(--glass-border)] relative z-10 bg-[var(--bg-primary)] text-[var(--text-primary)]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-quicksand font-bold tracking-tight text-[var(--text-primary)]">Edit Product</h1>
            </div>
          </div>
          <div className="flex items-center gap-4">
