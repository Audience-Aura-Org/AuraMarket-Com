    ...
    <div className="relative min-h-full bg-[var(--bg-secondary)] text-[var(--text-primary)] transition-colors duration-500 overflow-x-hidden">
        ...
        <header className="h-14 sm:h-16 flex items-center justify-between px-3 sm:px-6 lg:px-8 glass-panel border-b border-[var(--glass-border)] relative z-20 bg-[var(--bg-primary)] backdrop-blur-2xl gap-2 min-w-0 text-[var(--text-primary)]">
          <div className="flex items-center gap-3 sm:gap-5 min-w-0 flex-1">
            ...
            <p className="hidden md:block text-[var(--text-secondary)] text-[9px] font-bold tracking-tight  opacity-60">Status: <span className="text-[var(--text-primary)]">Online</span></p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-6 space-y-6 no-scrollbar relative z-10">
              ...
              return (
                <div key={stat.label} className="glass-panel p-5 rounded-[2rem] hover:-translate-y-1 transition-all duration-500 bg-[var(--bg-primary)]/60 border border-[var(--glass-border)] shadow-sm hover:shadow-xl group">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {stat.pct ? (
                      <span className={`text-[9px] font-bold px-3 py-1 rounded-full ${c.badgeBg} ${c.badgeText} tracking-tight `}>{stat.pct}</span>
                    ) : (
                  ...
                  <p className="text-[var(--text-secondary)] text-[9px] font-bold tracking-tight  opacity-40 mb-1">{stat.label}</p>
                  ...
                  {stat.color !== 'purple' ? (
                    ...
                    <div className="mt-4 flex gap-2">
                      <Link href="/wallet" className="flex-1 bg-[var(--accent)] hover:opacity-90 text-white text-[9px] font-bold py-2 rounded-full shadow-lg shadow-[var(--accent)]/20 transition-all tracking-tight active:scale-95 text-center">Withdraw</Link>
                      <Link href="/wallet" className="flex-1 glass-panel hover:bg-[var(--accent)]/5 text-[var(--text-primary)] text-[9px] font-bold py-2 rounded-full transition-all text-center border border-[var(--glass-border)] tracking-tight active:scale-95 flex items-center justify-center">Wallet</Link>
                    </div>
                  )}
                </div>
              );
          ...
          <div className="glass-panel p-6 rounded-[2rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/50">
            <p className="text-[9px] font-bold tracking-tight  text-[var(--text-secondary)] mb-4">Finance Intelligence</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div className="rounded-2xl border border-[var(--glass-border)] p-4 bg-[var(--bg-secondary)]/20">
                <p className="text-[9px] font-bold tracking-tight text-[var(--text-secondary)] opacity-50 mb-1">Available</p>
                <p className="font-bold text-[var(--text-primary)] text-lg tracking-tighter">{walletBalance.toLocaleString()} XAF</p>
              </div>
              <div className="rounded-2xl border border-[var(--glass-border)] p-4 bg-[var(--bg-secondary)]/20">
                <p className="text-[9px] font-bold tracking-tight text-[var(--text-secondary)] opacity-50 mb-1">In Escrow</p>
                <p className="font-bold text-[var(--text-primary)] text-lg tracking-tighter">{pendingEscrow.toLocaleString()} XAF</p>
              </div>
              <div className="rounded-2xl border border-[var(--glass-border)] p-4 bg-[var(--bg-secondary)]/20">
                <p className="text-[9px] font-bold tracking-tight text-[var(--text-secondary)] opacity-50 mb-1">Recent Paid</p>
          ...
          <div className="glass-panel p-6 rounded-[2rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/50 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link href="/vendor/stories" className="px-6 py-3 bg-[var(--accent)] text-white rounded-full text-[9px] font-bold tracking-tight shadow-lg shadow-[var(--accent)]/20 hover:scale-105 transition-all">
              Launch Story Hub
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 glass-panel rounded-[2rem] p-6 border border-[var(--glass-border)] bg-[var(--bg-primary)]/50 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <select className="bg-[var(--bg-primary)] border border-[var(--glass-border)] text-[var(--text-primary)] text-[9px] font-bold rounded-full px-4 py-2 focus:outline-none tracking-tight">
              ...
              <div className="flex justify-between mt-3 px-1 text-[9px] font-bold text-[var(--text-secondary)] tracking-tight  opacity-40">
            ...
            <div className="glass-panel rounded-[2rem] p-6 flex flex-col border border-[var(--glass-border)] bg-[var(--bg-primary)]/50">
              <div className="flex justify-between items-center mb-6">
                ...
                <Link href="/vendor/orders" className="text-[var(--accent)] text-[9px] font-bold tracking-tight hover:underline">View All</Link>
              </div>
          ...
          <div className="glass-panel rounded-[2rem] overflow-hidden border border-[var(--glass-border)] bg-[var(--bg-primary)]/50 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[9px] font-bold tracking-tight text-[var(--text-secondary)] bg-[var(--bg-secondary)]/50">
                    ...
                    return (
                      <tr key={order._id || i} className="hover:bg-[var(--accent)]/5 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-[9px] font-bold tracking-tight ${statusStyles[status] || statusStyles.processing}`}>
