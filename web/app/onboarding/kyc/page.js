      ...
      <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex items-center justify-center p-6 md:p-12 relative overflow-hidden">
         ...
         >
            ...
            <div className="w-full md:w-80 bg-[var(--bg-primary)]/40 border-r border-[var(--glass-border)] p-12 flex flex-col gap-12">
               <button onClick={() => router.back()} className="flex items-center gap-2 text-[11px] lg:text-[12px] font-bold tracking-tight  opacity-40 hover:opacity-100 transition-opacity">
               ...
               <div className="space-y-4">
                  ...
                  <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-60 tracking-tight">Aura Market Partner Compliance v6.0</p>
               </div>
               ...
               <nav className="space-y-8 mt-12">
                  {steps.map((s, i) => (
                     <div key={s.id} className={`flex items-start gap-4 transition-all ${step >= s.id ? 'opacity-100' : 'opacity-20'}`}>
                        <div className={`size-8 rounded-xl flex items-center justify-center text-[11px] lg:text-[12px] font-bold border ${step === s.id ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--glass-border)]'}`}>
                           {step > s.id ? <CheckCircle2 className="size-4" /> : s.id}
                        </div>
                        <div className="space-y-1">
                           <span className="text-[11px] lg:text-[12px] font-bold tracking-tight block">{s.title}</span>
                           <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] ">{s.desc}</p>
                        </div>
                     </div>
                  ))}
               </nav>
               ...
               <div className="mt-auto space-y-4 border-t border-[var(--glass-border)] pt-8">
                  <div className="flex items-center gap-3 text-[11px] lg:text-[12px] font-bold tracking-tight  opacity-40">
                     ...
                     >
                        ...
                        <div className="space-y-6">
                           <div className="space-y-3">
                              <label className="text-[11px] lg:text-[12px] font-bold tracking-tight text-[var(--text-secondary)] ml-2">ID Protocol Type</label>
                        ...
                        <button
                           onClick={() => setStep(2)}
                           className="w-full py-6 rounded-3xl bg-[var(--text-primary)] text-[var(--bg-primary)] font-bold  tracking-[.3em] text-[10px] lg:text-[12px] flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-95 transition-all shadow-xl"
                        >
                  ...
                  {step === 2 && (
                     ...
                     >
                        ...
                        <div className="p-12 rounded-[40px] border-2 border-dashed border-[var(--glass-border)] bg-[var(--bg-primary)] shadow-inner flex flex-col items-center justify-center text-center gap-6 group hover:border-[var(--accent)]/30 transition-all cursor-pointer">
                           ...
                           <div className="space-y-1">
                              <span className="text-[11px] lg:text-[12px] font-bold tracking-tight block">Drop Proof Artifact Here</span>
                              <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)]  opacity-40">PDF / JPEG / PNG MAX 10MB</p>
                           </div>
                        </div>

                        <div className="flex gap-4">
                           <button onClick={() => setStep(1)} className="flex-1 py-6 rounded-3xl border border-[var(--glass-border)] font-bold tracking-tight text-[10px] lg:text-[12px] hover:bg-[var(--bg-secondary)] transition-all">Previous</button>
                           <button onClick={() => setStep(3)} className="flex-[2] py-6 rounded-3xl bg-[var(--text-primary)] text-[var(--bg-primary)] font-bold  tracking-[.3em] text-[10px] lg:text-[12px] shadow-xl hover:scale-[1.02] transition-all">Continue Sync</button>
                        </div>
                     </motion.div>
                  )}
                  ...
                  {step === 3 && (
                     ...
                     >
                        ...
                        <button
                           ...
                           className="w-full py-8 rounded-[32px] bg-[var(--accent)] text-white font-bold  tracking-[.4em] text-[11px] lg:text-[12px] flex items-center justify-center gap-4 hover:scale-[1.02] shadow-[0_20px_60px_rgba(242,13,242,0.3)] transition-all disabled:opacity-50"
                        >
...
function UploadCard({ label, icon: Icon }) {
   return (
      <div className="flex flex-col items-center justify-center p-8 rounded-3xl bg-[var(--bg-primary)] border border-[var(--glass-border)] cursor-pointer hover:border-[var(--accent)]/30 group transition-all">
         ...
         <span className="text-[11px] lg:text-[12px] font-bold tracking-tight text-center">{label}</span>
      </div>
   );
}
