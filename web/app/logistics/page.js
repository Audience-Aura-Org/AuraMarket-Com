        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between items-start gap-4">
           <div className="space-y-4">
              <div className="flex items-center gap-2 text-[var(--accent)] bg-white/5 px-3 py-1 rounded-full border border-white/10 w-fit">
                 ...
                 <span className="text-[11px] lg:text-[12px] font-bold  tracking-[0.4em]">Fulfillment Sync</span>
              </div>
           ...
           <div className="flex gap-2">
              <Link href="/logistics/tracking" className="h-10 px-6 bg-[var(--accent)] text-white rounded-xl text-[11px] lg:text-[12px] font-bold tracking-tight flex items-center gap-3 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-[var(--accent)]/10">
                 Track Signal <RefreshCw className="size-3" />
              </Link>
              <Link href="/logistics/dashboard" className="h-10 px-6 bg-white/5 border border-white/10 rounded-xl text-[11px] lg:text-[12px] font-bold tracking-tight flex items-center gap-2 hover:bg-white/10 transition-all text-[var(--text-primary)]">
                 Dashboard
              </Link>
           </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           {[
              ...
              <div key={stat.label} className="p-5 rounded-2xl bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] flex flex-col gap-4 group">
                 ...
                 <div>
                    <h4 className="text-[11px] lg:text-[12px] font-bold tracking-tight text-[var(--text-secondary)] opacity-40 ">{stat.label}</h4>
              ...
              <div className="space-y-2">
                 {[
                    ...
                    <div key={item.node} className="p-4 rounded-xl bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] flex items-center justify-between gap-6 hover:bg-[var(--bg-secondary)] transition-all">
                       <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-primary)] opacity-60 ">{item.node}</p>
                       <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-[var(--accent)] transition-all duration-1000" style={{ width: `${item.progress}%` }} />
                       </div>
                       <p className="text-[11px] lg:text-[12px] font-bold text-[var(--accent)] tracking-tight w-20 text-right">{item.status}</p>
                    </div>
                 ))}
              </div>
           </div>
