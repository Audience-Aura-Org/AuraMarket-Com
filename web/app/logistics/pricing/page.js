...
export default function LogisticsPricingPage() {
  ...
  return (
    <>
      <header className="h-20 lg:h-24 flex flex-col lg:flex-row lg:items-center justify-between px-6 lg:px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] backdrop-blur-3xl shrink-0 z-10 py-4 lg:py-0 gap-4 lg:gap-0 text-[var(--text-primary)]">
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          <div>
             <h2 className="text-lg lg:text-2xl font-quicksand font-bold tracking-tighter  leading-none">Route <span className="text-[var(--accent)]">Pricing</span></h2>
             <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-[0.3em]  opacity-40 mt-1">Matrix Protocol</p>
          </div>
        </div>
      </header>
      ...
      <div className="p-4 lg:p-10 space-y-8 lg:space-y-12 pb-32">
          <section className="space-y-4 lg:space-y-6">
             <div className="flex items-center gap-3 lg:gap-4">
                ...
                <h3 className="text-lg lg:text-2xl font-quicksand font-bold tracking-tighter  leading-none">Last-Mile Rates</h3>
             </div>
                ...
                <div className="lg:col-span-4 space-y-6">
                   <div className="glass-panel p-6 lg:p-8 rounded-[32px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] ml-1 opacity-50">Operation Region</label>
                        <select value={selectedDistrict} onChange={e => { setSelectedDistrict(e.target.value); setSelectedQuartiers([]); }} className="w-full px-4 py-3.5 lg:px-6 lg:py-4 rounded-xl lg:rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] outline-none text-[10px] lg:text-[12px] lg:text-xs font-quicksand font-bold transition-all">
                      ...
                      {selectedDistrict && (
                         <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="flex justify-between items-center px-1">
                               <label className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] opacity-40">Target Zones</label>
                               <div className="flex gap-3">
                                  <button onClick={() => setSelectedQuartiers(zones.filter(z => z.type === 'quartier' && z.parent_id?._id === selectedDistrict).map(z => z.name))} className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--accent)] hover:opacity-100 transition-opacity">Select All</button>
                                  <button onClick={() => setSelectedQuartiers([])} className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-rose-500 hover:opacity-100 transition-opacity">Clear</button>
                               </div>
                            </div>
                            <div className="max-h-[160px] lg:max-h-[200px] overflow-y-auto pr-2 space-y-2 no-scrollbar border-t border-b border-[var(--glass-border)] py-2">
                               {zones.filter(z => z.type === 'quartier' && z.parent_id?._id === selectedDistrict).map(z => (
                                 <div key={z._id} onClick={() => setSelectedQuartiers(prev => prev.includes(z.name) ? prev.filter(n => n !== z.name) : [...prev, z.name])} className={`p-3 lg:p-4 rounded-xl lg:rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${selectedQuartiers.includes(z.name) ? 'bg-[var(--accent)]/10 border-[var(--accent)]/40' : 'bg-transparent border-transparent hover:bg-[var(--bg-primary)]/40'}`}>
                                    <span className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight">{z.name}</span>
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] ml-1 opacity-50">Base Fee</label>
                         <div className="relative">
                            <input type="number" value={newPrice} onChange={e => setNewPrice(e.target.value)} className="w-full pl-6 pr-16 py-3.5 lg:py-4 rounded-xl lg:rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] outline-none text-[10px] lg:text-[12px] lg:text-xs font-quicksand font-bold transition-all" />
                            <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] opacity-30 font-quicksand font-bold tracking-tight">XAF</span>
                         </div>
                      </div>
                      <button onClick={handleAddQuartier} className="w-full py-4 lg:py-5 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-xl lg:rounded-2xl font-quicksand font-bold text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] tracking-[0.3em]  shadow-2xl hover:bg-[var(--accent)] hover:text-white hover:-translate-y-1 transition-all">Add Routes <Plus className="size-4 ml-2 inline" /></button>
                   </div>
                </div>
                ...
                <div className="lg:col-span-8 flex flex-col gap-4 lg:gap-6">
                   <div className="glass-panel rounded-[28px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl relative min-h-[500px] flex flex-col">
                      <div className="p-5 lg:p-8 border-b border-[var(--glass-border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
                         <div className="flex items-center gap-4 flex-1">
                            ...
                            <input placeholder="Search zone network..." value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="bg-transparent border-none outline-none text-[10px] lg:text-[12px] lg:text-xs font-quicksand font-bold tracking-tight w-full" />
                         </div>
                         <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold  opacity-40 tracking-[0.2em]">{profile.quartier_prices.length} Active Nodes</p>
                      </div>
                      {tableSelections.length > 0 && (
                         <div className="p-3 lg:p-5 bg-[var(--accent)]/10 border-b border-[var(--glass-border)] flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-500">
                            <div className="flex items-center gap-4 flex-1">
                               <label className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold  opacity-40 tracking-tight whitespace-nowrap">Bulk Rate:</label>
                               <input type="number" placeholder="Enter fee..." value={bulkUpdatePrice} onChange={e => setBulkUpdatePrice(e.target.value)} className="w-full md:w-32 px-4 py-2.5 rounded-xl bg-[var(--bg-primary)] border border-[var(--glass-border)] text-[10px] lg:text-[12px] lg:text-xs font-quicksand font-bold outline-none shadow-inner" />
                            </div>
                            <div className="flex gap-3">
                               <button onClick={handleBulkUpdatePrice} className="flex-1 md:flex-none px-6 py-2.5 bg-[var(--accent)] text-white text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight rounded-xl shadow-lg hover:scale-[1.02] transition-all">Apply</button>
                               <button onClick={handleBulkDelete} className="flex-1 md:flex-none px-6 py-2.5 bg-rose-500/10 text-rose-600 text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight rounded-xl hover:bg-rose-500 hover:text-white transition-all">Remove</button>
                            </div>
                         </div>
                      )}
                      <div className="flex-1 overflow-x-auto no-scrollbar">
                         <table className="w-full text-left border-collapse">
                               ...
                               <tr>
                                  ...
                                  <th className="px-6 lg:px-8 py-3 lg:py-4 text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold opacity-30  tracking-[0.2em]">Transit Zone</th>
                                  <th className="px-6 lg:px-8 py-3 lg:py-4 text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold opacity-30  tracking-[0.2em]">Operational Fee</th>
                             ...
                             <tbody className="divide-y divide-[var(--glass-border)]/10">
                                {currentPrices.map((q, idx) => (
                                  <tr key={idx} className={`hover:bg-[var(--accent)]/5 transition-all group ${tableSelections.includes(q.quartier) ? 'bg-[var(--accent)]/5' : ''}`}>
                                     ...
                                     <td className="px-6 lg:px-8 py-4 lg:py-5 border-none"><p className="text-[10px] lg:text-[12px] lg:text-sm font-quicksand font-bold tracking-tight text-[var(--text-primary)]">{q.quartier}</p></td>
                                     <td className="px-6 lg:px-8 py-4 lg:py-5 border-none">
                                        <div className="flex items-center gap-2 group/input">
                                           <input 
                                              ...
                                              className="w-20 lg:w-28 px-3 py-2 bg-[var(--bg-secondary)]/30 border border-transparent hover:border-[var(--glass-border)] focus:border-[var(--accent)] rounded-lg text-xs lg:text-sm font-mono font-quicksand font-bold text-indigo-600 outline-none transition-all shadow-inner" 
                                           />
                                           <span className="text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] opacity-20 font-quicksand font-bold">XAF</span>
                                        </div>
                                     </td>
                   ...
                   <div className="flex justify-center pt-2 lg:pt-4">
                      <button 
                        onClick={handleSave} 
                        disabled={saving} 
                        className="w-full md:w-auto px-8 lg:px-12 py-4 lg:py-5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-2xl lg:rounded-[24px] font-quicksand font-bold text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] tracking-[0.3em]  shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-40"
                      >
