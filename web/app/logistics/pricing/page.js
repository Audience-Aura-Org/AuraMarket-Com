      ...
      <header className="h-20 lg:h-24 flex flex-col lg:flex-row lg:items-center justify-between px-6 lg:px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] backdrop-blur-3xl shrink-0 z-10 py-4 lg:py-0 gap-4 lg:gap-0 text-[var(--text-primary)]">
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          <div>
             ...
             <p className="text-[7px] lg:text-[11px] font-bold tracking-[0.3em]  opacity-40 mt-1">Matrix Protocol</p>
          </div>
        </div>
      </header>
                ...
                <div className="lg:col-span-4 space-y-6">
                   <div className="glass-panel p-6 lg:p-8 rounded-[32px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 space-y-6">
                      <div className="space-y-2">
                        <label className="text-[8px] lg:text-[11px] font-bold tracking-tight text-[var(--text-secondary)] ml-1 opacity-50">Operation Region</label>
                      ...
                      {selectedDistrict && (
                         <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="flex justify-between items-center px-1">
                               <label className="text-[8px] lg:text-[11px] font-bold tracking-tight text-[var(--text-secondary)] opacity-40">Target Zones</label>
                               <div className="flex gap-3">
                                  <button onClick={() => setSelectedQuartiers(zones.filter(z => z.type === 'quartier' && z.parent_id?._id === selectedDistrict).map(z => z.name))} className="text-[8px] lg:text-[11px] font-bold tracking-tight text-[var(--accent)] hover:opacity-100 transition-opacity">Select All</button>
                                  <button onClick={() => setSelectedQuartiers([])} className="text-[8px] lg:text-[11px] font-bold tracking-tight text-rose-500 hover:opacity-100 transition-opacity">Clear</button>
                               </div>
                            </div>
                            <div className="max-h-[160px] lg:max-h-[200px] overflow-y-auto pr-2 space-y-2 no-scrollbar border-t border-b border-[var(--glass-border)] py-2">
                               {zones.filter(z => z.type === 'quartier' && z.parent_id?._id === selectedDistrict).map(z => (
                                 <div key={z._id} onClick={() => setSelectedQuartiers(prev => prev.includes(z.name) ? prev.filter(n => n !== z.name) : [...prev, z.name])} className={`p-3 lg:p-4 rounded-xl lg:rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${selectedQuartiers.includes(z.name) ? 'bg-[var(--accent)]/10 border-[var(--accent)]/40' : 'bg-transparent border-transparent hover:bg-[var(--bg-primary)]/40'}`}>
                                    <span className="text-[9px] lg:text-[11px] font-bold tracking-tight">{z.name}</span>
                      ...
                      <div className="space-y-2">
                         <label className="text-[8px] lg:text-[11px] font-bold tracking-tight text-[var(--text-secondary)] ml-1 opacity-50">Base Fee</label>
                         <div className="relative">
                            ...
                            <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[8px] lg:text-[10px] opacity-30 font-bold tracking-tight">XAF</span>
                         </div>
                      </div>
                      <button onClick={handleAddQuartier} className="w-full py-4 lg:py-5 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-xl lg:rounded-2xl font-bold text-[9px] lg:text-[10px] tracking-[0.3em]  shadow-2xl hover:bg-[var(--accent)] hover:text-white hover:-translate-y-1 transition-all">Add Routes <Plus className="size-4 ml-2 inline" /></button>
                   </div>
                </div>
                ...
                <div className="lg:col-span-8 flex flex-col gap-4 lg:gap-6">
                   <div className="glass-panel rounded-[28px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl relative min-h-[500px] flex flex-col">
                      <div className="p-5 lg:p-8 border-b border-[var(--glass-border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
                         ...
                         <p className="text-[8px] lg:text-[11px] font-bold  opacity-40 tracking-[0.2em]">{profile.quartier_prices.length} Active Nodes</p>
                      </div>
                      {tableSelections.length > 0 && (
                         <div className="p-3 lg:p-5 bg-[var(--accent)]/10 border-b border-[var(--glass-border)] flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-500">
                            <div className="flex items-center gap-4 flex-1">
                               <label className="text-[8px] lg:text-[11px] font-bold  opacity-40 tracking-tight whitespace-nowrap">Bulk Rate:</label>
                            ...
                            <div className="flex gap-3">
                               <button onClick={handleBulkUpdatePrice} className="flex-1 md:flex-none px-6 py-2.5 bg-[var(--accent)] text-white text-[9px] lg:text-[11px] font-bold tracking-tight rounded-xl shadow-lg hover:scale-[1.02] transition-all">Apply</button>
                               <button onClick={handleBulkDelete} className="flex-1 md:flex-none px-6 py-2.5 bg-rose-500/10 text-rose-600 text-[9px] lg:text-[11px] font-bold tracking-tight rounded-xl hover:bg-rose-500 hover:text-white transition-all">Remove</button>
                            </div>
                         </div>
                      )}
                      <div className="flex-1 overflow-x-auto no-scrollbar">
                         <table className="w-full text-left border-collapse">
                               ...
                               <tr>
                                  ...
                                  <th className="px-6 lg:px-8 py-3 lg:py-4 text-[7px] lg:text-[11px] font-bold opacity-30  tracking-[0.2em]">Transit Zone</th>
                                  <th className="px-6 lg:px-8 py-3 lg:py-4 text-[7px] lg:text-[11px] font-bold opacity-30  tracking-[0.2em]">Operational Fee</th>
                                  ...
                                  <tr key={idx} className={`hover:bg-[var(--accent)]/5 transition-all group ${tableSelections.includes(q.quartier) ? 'bg-[var(--accent)]/5' : ''}`}>
                                     ...
                                     <td className="px-6 lg:px-8 py-4 lg:py-5 border-none">
                                        <div className="flex items-center gap-2 group/input">
                                           ...
                                           />
                                           <span className="text-[8px] lg:text-[9px] opacity-20 font-bold">XAF</span>
                                        </div>
                                     </td>
                   ...
                   <div className="flex justify-center pt-2 lg:pt-4">
                      <button 
                        onClick={handleSave} 
                        disabled={saving} 
                        className="w-full md:w-auto px-8 lg:px-12 py-4 lg:py-5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-2xl lg:rounded-[24px] font-bold text-[9px] lg:text-[10px] tracking-[0.3em]  shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-40"
                      >
