  ...
  return (
    <div className="p-4 lg:p-10 space-y-6">
      <div>
        ...
        <p className="text-[10px] lg:text-xs font-bold tracking-tight opacity-60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-4 lg:col-span-2">
          <p className="text-[11px] font-bold tracking-tight opacity-50">Logistics Company</p>
        ...
        <div className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-4">
          <p className="text-[11px] font-bold tracking-tight opacity-50">Pricing Coverage</p>
          <p className="text-2xl font-bold">{firm?.quartier_prices?.length || 0}</p>
          <p className="text-[11px] font-bold  opacity-60">Configured quartiers</p>
          <Link href="/logistics/pricing" className="mt-4 inline-block rounded-lg bg-[var(--text-primary)] px-3 py-2 text-[11px] font-bold  text-[var(--bg-primary)]">
            Manage pricing
          </Link>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-4 lg:p-6">
        <div className="flex items-center justify-between mb-6">
           <p className="text-[11px] font-bold tracking-tight opacity-60">Available towns and quartiers</p>
           <p className="text-[11px] font-bold opacity-40 tracking-tight">{zones.length} Total Signals</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 min-h-[200px]">
          {currentZones.map((z) => (
            <div key={z._id} className="rounded-lg border border-[var(--glass-border)] px-3 py-2 hover:bg-[var(--accent)]/5 transition-colors">
              ...
              <p className="text-[11px] font-bold opacity-60 ">{z.type}</p>
            </div>
          ))}
          {!zones.length && <p className="text-[11px] font-bold  opacity-40">No zones found</p>}
        </div>
