        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] tracking-tight opacity-40">Operational Analytics</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            {['7', '30', '90'].map(t => (
              <button
                key={t}
                onClick={() => setRange(t)}
                className={`px-5 py-2 rounded-full text-[11px] lg:text-[12px] font-bold  transition-all tracking-tight ${
      ...
      <div className="max-w-[1600px] mx-auto p-4 md:p-8 space-y-8">
        ...
        <section className="relative p-8 rounded-[2rem] bg-[var(--bg-secondary)]/10 border border-[var(--glass-border)]">
          <div className="flex items-center justify-between mb-12">
            ...
            <div className="flex gap-4">
               <div className="flex flex-col items-end">
                  <span className="text-[11px] lg:text-[12px] font-bold opacity-30 ">Peak Pulse</span>
                ...
                <AnimatePresence>
                  {hoveredBlock === d && (
                    <motion.div
                      ...
                      className="absolute bottom-full left-1/2 -translate-x-1/2 mb-6 px-4 py-2 rounded-full bg-[var(--text-primary)] text-[var(--bg-primary)] text-[11px] lg:text-[12px] font-bold whitespace-nowrap z-50 shadow-2xl border border-[var(--glass-border)]"
                    >
                      {d.count} SHIPMENTS
                      <span className="block text-[11px] lg:text-[12px] font-bold opacity-60 ">{d.label}</span>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
          ...
          <div className="flex justify-between mt-6 px-2">
            <span className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-30 tracking-tight">{histogramData[0]?.label}</span>
            <span className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-30 tracking-tight">{histogramData[29]?.label}</span>
          </div>
        </section>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <section className="p-8 rounded-[2rem] bg-[var(--bg-secondary)]/10 border border-[var(--glass-border)] flex flex-col justify-center items-center text-center shadow-sm">
            ...
            <button className="px-8 py-3 rounded-full bg-[var(--text-primary)] text-[var(--bg-primary)] text-[11px] lg:text-[12px] font-bold  tracking-[0.2em] hover:bg-[var(--accent)] hover:text-white transition-all shadow-xl shadow-[var(--accent)]/10 active:scale-95">
              Execute Rebalance
            </button>
          </section>
        </div>
