      ...
      <header className="px-4 md:px-8 py-6 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-quicksand font-bold tracking-tighter">Logistics Hub</h1>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight opacity-60">Network Operational</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex flex-col items-end">
              <p className="text-xs font-quicksand font-bold">{new Date().toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' })}</p>
              <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] opacity-40  font-quicksand font-bold">GMT +1</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            >
              ...
              <p className="text-[11px] lg:text-[12px] font-quicksand font-bold opacity-40 tracking-tight">{stat.sub}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 p-8 rounded-3xl bg-[var(--bg-primary)] border border-[var(--glass-border)] relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-xl font-quicksand font-bold tracking-tighter">Node Saturation</h3>
                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] opacity-60">Regional manifest distribution</p>
              </div>
              <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--accent)] text-white text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight hover:shadow-[0_0_20px_rgba(var(--accent-rgb),0.4)] transition-all">
          ...
          <div className="space-y-4">
            <div className="p-6 rounded-3xl bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)]">
              <h3 className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight mb-4">Quick Command</h3>
              <div className="grid grid-cols-2 gap-3">
                <button className="p-4 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)] transition-all flex flex-col items-center gap-2 group">
                  ...
                  <span className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight">New Manifest</span>
                </button>
                <button className="p-4 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)] transition-all flex flex-col items-center gap-2 group">
                  <MapPin className="w-5 h-5 text-blue-500 group-hover:scale-110 transition-transform" />
                  <span className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight">Manage Nodes</span>
                </button>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl bg-[var(--bg-primary)] border border-[var(--glass-border)]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight">Recent Alarms</h3>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between text-[10px] lg:text-[12px]">
                    <span className="font-quicksand font-bold tracking-tight opacity-60">{item.msg}</span>
                    <span className="opacity-30 whitespace-nowrap font-quicksand font-bold tracking-tight">{item.time}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="p-8 rounded-3xl bg-[var(--bg-secondary)]/10 border border-[var(--glass-border)]">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-quicksand font-bold tracking-tighter ">Shipment Stream</h3>
            </div>
            
            <div className="flex gap-2">
              <button className="px-4 py-2 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight flex items-center gap-2 hover:bg-white/5 transition-all">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] opacity-40">
                ...
                {shipments.slice(0, 8).map((s) => (
                  <tr key={s._id} className="group hover:bg-white/[0.02] transition-colors">
                    <td className="py-4 pr-4">
                      <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tighter ">#{s._id?.slice(-8).toUpperCase()}</p>
                      <p className="text-[10px] lg:text-[12px] opacity-40  font-quicksand font-bold tracking-tight">{s.tracking_number || 'TRK-PENDING'}</p>
                    </td>
                    <td className="py-4 pr-4">
                      <p className="text-xs font-quicksand font-bold truncate max-w-[150px]">{s.destination_address?.city || 'Unknown'}</p>
                      <p className="text-[10px] lg:text-[12px] opacity-40  font-quicksand font-bold">{s.destination_address?.region || 'N/A'}</p>
                    </td>
                    <td className="py-4 pr-4">
                      <span className={`px-3 py-1 rounded-full text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight ${
                    ...
                    <td className="py-4 pr-4 text-xs font-quicksand font-bold italic opacity-60">High</td>
