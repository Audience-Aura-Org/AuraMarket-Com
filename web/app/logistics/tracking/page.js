...
export default function LogisticsTrackingPage() {
  ...
  return (
    <div className="p-4 lg:p-10 space-y-6">
      <div>
        <h1 className="text-lg lg:text-2xl font-quicksand font-bold tracking-tight">Live Tracking</h1>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {cards.map(([label, value]) => (
              <div key={label} className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-4">
                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight opacity-50">{label}</p>
                <p className="text-xl font-quicksand font-bold">{value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 p-4 lg:p-6 min-h-[400px] flex flex-col">
            <div className="flex items-center justify-between mb-6">
               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight opacity-60">Recent activity</p>
               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold opacity-40 tracking-tight">{shipments.length} Total Logs</p>
            </div>
            ...
            <div className="space-y-2 flex-1">
              {currentShipments.map((s) => (
                <div key={s._id} className="flex items-center justify-between rounded-xl border border-[var(--glass-border)] px-3 py-2 hover:bg-[var(--accent)]/5 transition-colors">
                  <div>
                    <p className="text-xs font-quicksand font-bold">{s.tracking_code}</p>
                    <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--accent)] ">
                      Order #{(s.order_id?._id || s.order_id || "").toString().slice(-8).toUpperCase()}
                    </p>
                    <p className="text-[11px] lg:text-[12px] font-quicksand font-bold opacity-60 truncate max-w-[200px]">
                      {s.delivery_address?.quartier || s.delivery_address?.city || "Unknown destination"}
                    </p>
                  </div>
                  <span className="text-[11px] lg:text-[12px] font-quicksand font-bold  bg-[var(--bg-secondary)] border border-[var(--glass-border)] px-2.5 py-1 rounded-lg">{(s.status || "").replace(/_/g, " ")}</span>
                </div>
              ))}
              {!shipments.length && <p className="text-[11px] lg:text-[12px] font-quicksand font-bold  opacity-40 py-20 text-center">No shipment activity yet</p>}
            </div>
