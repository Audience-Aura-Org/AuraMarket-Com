...
export default function RewardsPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-secondary)] text-[var(--text-primary)] overflow-hidden pb-24 transition-colors duration-500 pt-14 md:pt-0">
      {/* Gradients */}
        ...
        <main className="px-8 space-y-12 mt-4">
          ...
          <section>
            <div className="flex items-center justify-between mb-8 px-2">
              ...
              <button className="text-[11px] lg:text-[12px]  font-semibold tracking-[0.2em] text-[var(--accent)] hover:underline transition-all ">View Matrix</button>
            </div>
  ...
  return (
    <div className={`min-w-[200px] bg-[var(--bg-primary)] border border-[var(--glass-border)] p-8 rounded-[40px] flex flex-col gap-6 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 ${disabled ? 'opacity-40 grayscale-[0.5]' : ''}`}>
      ...
      <div>
        ...
        <p className="text-[11px] lg:text-[12px]  font-semibold tracking-tight text-[var(--text-secondary)] mt-2  opacity-60">{points} AUR Points</p>
      </div>
      <button disabled={disabled} className={`w-full py-4 text-[11px] lg:text-[12px]  font-semibold tracking-tight rounded-2xl transition-all  mt-2 shadow-sm ${color} ${!disabled && 'hover:scale-95'}`}>
        {action}
      </button>
    </div>
  );
    ...
    <div className="flex items-center justify-between p-8 bg-[var(--bg-primary)]/80 border border-[var(--glass-border)] rounded-[32px] hover:bg-[var(--bg-primary)] transition-all shadow-sm group">
      <div className="flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-[11px] lg:text-[12px]  font-semibold tracking-tight text-[var(--text-secondary)] mt-1.5  opacity-30">{subtitle}</p>
        </div>
      </div>
