      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <div className="sticky top-0 z-50 bg-[var(--bg-secondary)]/80 backdrop-blur-xl px-8 py-8 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <h1 className="text-4xl font-quicksand font-bold tracking-tighter  text-[var(--text-primary)]">Aura Rewards</h1>
          </div>
        ...
        <main className="px-8 space-y-12 mt-4">
          ...
          <section className="relative overflow-hidden p-12 rounded-[48px] bg-[var(--bg-primary)]/80 border border-[var(--accent)]/20 shadow-2xl glass-panel">
            <div className="relative z-10">
              <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--accent)] tracking-[0.4em] mb-4  opacity-80">Accumulated Points</p>
              <div className="flex items-baseline gap-3">
                <h2 className="text-7xl font-quicksand font-bold text-[var(--text-primary)] tracking-tighter font-mono">2,840</h2>
                <span className="text-3xl font-quicksand font-bold text-[var(--accent)]">AUR</span>
              </div>
              <div className="mt-12 flex items-center gap-6">
                ...
                <span className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] whitespace-nowrap ">70% to Gold Tier</span>
              </div>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-8 px-2">
              <h3 className="text-2xl font-quicksand font-bold tracking-tight ">Redeem Nexus</h3>
              <button className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-[0.2em] text-[var(--accent)] hover:underline transition-all ">View Matrix</button>
            </div>
          ...
          <section className="bg-[var(--bg-primary)]/40 border border-dashed border-[var(--accent)]/30 rounded-[56px] p-12 hover:bg-[var(--bg-primary)]/60 transition-all duration-500">
            <div className="flex flex-col items-center justify-center text-center gap-8">
              ...
              <div className="space-y-4">
                <h3 className="text-4xl font-quicksand font-bold tracking-tight ">Expansion Protocol</h3>
              ...
              <div className="w-full flex items-center gap-4 mt-8">
                ...
                >
                  <span className="font-mono font-quicksand font-bold text-[var(--accent)] tracking-tight text-xl">
          ...
          <section className="pb-12">
            <h3 className="text-2xl font-quicksand font-bold tracking-tight mb-8 px-2 ">Sync Activity</h3>
            <div className="space-y-4">
              <ActivityRow 
...
function RewardCard({ icon: Icon, title, points, action, color, disabled }) {
  return (
    <div className={`min-w-[200px] bg-[var(--bg-primary)] border border-[var(--glass-border)] p-8 rounded-[40px] flex flex-col gap-6 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 ${disabled ? 'opacity-40 grayscale-[0.5]' : ''}`}>
      ...
      <div>
        <p className="text-xl font-quicksand font-bold text-[var(--text-primary)] tracking-tight">{title}</p>
        <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] mt-2  opacity-60">{points} AUR Points</p>
      </div>
      <button disabled={disabled} className={`w-full py-4 text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight rounded-2xl transition-all  mt-2 shadow-sm ${color} ${!disabled && 'hover:scale-95'}`}>
        {action}
      </button>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-between p-8 bg-[var(--bg-primary)]/80 border border-[var(--glass-border)] rounded-[32px] hover:bg-[var(--bg-primary)] transition-all shadow-sm group">
      <div className="flex items-center gap-6">
        ...
        <div>
          <p className="text-xl font-quicksand font-bold tracking-tight text-[var(--text-primary)] ">{title}</p>
          <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] mt-1.5  opacity-30">{subtitle}</p>
        </div>
      </div>
      <span className={`text-xl font-quicksand font-bold font-mono ${isPos ? 'text-emerald-600' : 'text-rose-600'}`}>
        {amount}
      </span>
    </div>
  );
