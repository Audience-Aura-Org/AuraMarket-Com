    ...
    <>
      ...
      <header className="h-16 flex items-center justify-between px-6 lg:px-10 glass-panel border-b border-[var(--glass-border)] z-20 bg-[var(--bg-primary)] backdrop-blur-xl shrink-0 text-[var(--text-primary)]">
        <div className="flex items-center gap-4">
          <h1 className="text-lg lg:text-xl font-quicksand font-bold tracking-tight ">Governance <span className="text-[var(--accent)]">Reviews</span></h1>
          <div className="hidden sm:block h-4 w-px bg-[var(--glass-border)] opacity-30" />
          <p className="hidden md:block text-[10px] lg:text-[12px] text-[var(--text-secondary)] font-quicksand font-bold tracking-tight">{reviews.length} System Records</p>
        </div>
      ...
      <div className="p-6 lg:p-10 space-y-8">
         ...
         <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:w-96 group">
              ...
              <input 
                ...
                className="w-full bg-[var(--bg-primary)]/50 border border-[var(--glass-border)] rounded-2xl pl-11 pr-4 py-3.5 text-xs font-quicksand font-bold focus:border-[var(--accent)] outline-none transition-all shadow-sm"
              />
            </div>
         </div>
         ...
         {loading ? (
           ...
           <div className="py-32 flex flex-col items-center justify-center text-center opacity-40">
              ...
              <h3 className="text-xl font-quicksand font-bold  tracking-tighter">No Feedback Records</h3>
              <p className="text-[11px] lg:text-[12px] font-quicksand font-bold  tracking-[0.2em] mt-2">The platform has not registered any reviews matching these criteria.</p>
           </div>
         ) : (
           <div className="glass-panel rounded-[24px] lg:rounded-[32px] border border-[var(--glass-border)] overflow-hidden bg-[var(--bg-primary)]/40 shadow-sm">
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                       <tr className="border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/60 text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)]">
                         ...
                         <tr key={r._id} className="hover:bg-[var(--bg-primary)]/60 transition-colors group">
                            <td className="px-8 py-6">
                               <div className="flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                     <p className="text-sm font-quicksand font-bold truncate">{r.user_id?.name || 'Anonymous'}</p>
                            ...
                            <td className="px-8 py-6">
                               <div className="flex items-center gap-3 max-w-[200px]">
                                  ...
                                  <p className="text-xs font-quicksand font-bold truncate">{r.product_id?.name || 'Deleted Product'}</p>
                               </div>
                            </td>
                            <td className="px-8 py-6">
                               <div className="flex items-center gap-1">
                                  ...
                                  <span className="ml-2 text-xs font-quicksand font-bold text-[var(--text-secondary)]">{r.rating}.0</span>
                               </div>
                            </td>
                            <td className="px-8 py-6">
