    ...
    <div className="min-h-screen bg-[var(--bg-primary)] py-12 px-6 md:px-12 transition-all duration-300">
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-[var(--glass-border)]">
           <div className="flex items-center gap-4">
              ...
              <div className="space-y-0.5">
                 ...
                 <p className="text-[10px] font-black  tracking-[0.2em] text-[var(--text-secondary)] opacity-40">System-wide Event Log</p>
              </div>
           </div>
                 ...
                 <div key={log.id} className="p-4 rounded-2xl bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] hover:border-[var(--accent)]/30 transition-all flex items-center gap-6 group">
                   ...
                   <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                         <h3 className="text-[10px] font-black text-[var(--text-primary)] tracking-tight">{log.action}</h3>
                         <span className="text-[9px] font-medium text-[var(--text-secondary)] opacity-30 tracking-wide">{log.time}</span>
                      </div>
                      <p className="text-xs text-[var(--text-secondary)] opacity-60 truncate font-mono">{log.details}</p>
                   </div>

                   <div className="hidden md:flex flex-col items-end shrink-0">
                      <span className="text-[9px] font-black text-[var(--text-secondary)] opacity-30 ">Operator</span>
                      <span className="text-[10px] font-bold text-[var(--text-primary)]">{log.user}</span>
                   </div>
                 </div>
               );
             })
        ...
        <div className="text-center pb-20 opacity-20">
           <p className="text-[8px] font-black tracking-[0.4em] text-[var(--text-secondary)] ">
