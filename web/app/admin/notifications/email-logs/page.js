      ...
      <div className="p-10 lg:px-20 space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-4 opacity-50">
              ...
              <span className="text-[11px] lg:text-[12px]  font-semibold tracking-tight ">Platform Intelligence</span>
            </div>
        ...
        <div className="glass-panel p-4 rounded-3xl border border-[var(--glass-border)] flex flex-col md:flex-row items-center gap-4 bg-[var(--bg-primary)]/40">
           <form onSubmit={handleSearch} className="flex-1 relative group w-full">
              ...
              <input 
                ...
                className="w-full pl-14 pr-6 py-4 rounded-2xl bg-[var(--bg-secondary)] border border-transparent focus:border-[var(--accent)]/30 outline-none text-[13px] placeholder:text-[11px] placeholder:font-normal font-bold transition-all"
              />
           </form>
           <div className="flex items-center gap-2 p-1 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--glass-border)]">
              {['all', 'sent', 'failed'].map(s => (
                <button
                  ...
                  className={`px-6 py-3 rounded-xl text-[11px] lg:text-[12px]  font-semibold tracking-tight transition-all ${status === s ? 'bg-[var(--accent)] text-white shadow-lg' : 'opacity-40 hover:opacity-100'}`}
                >
                  {s}
                </button>
              ))}
        ...
        <div className="glass-panel rounded-[40px] border border-[var(--glass-border)] overflow-hidden bg-[var(--bg-primary)]/20">
              ...
              <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30">
                       <th className="p-8 text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] opacity-40">Timestamp</th>
                       <th className="p-8 text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] opacity-40">Recipient Node</th>
                       <th className="p-8 text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] opacity-40">Subject / Role</th>
                       <th className="p-8 text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] opacity-40">Status</th>
                       <th className="p-8 text-[11px] lg:text-[12px]  font-semibold  tracking-[0.2em] opacity-40 text-right">Action</th>
                    </tr>
                 </thead>
                 <tbody className="divide-y divide-[var(--glass-border)]/50">
                    {loading ? (
                      ...
                      <tr key={log._id} className="group hover:bg-white/[0.02] transition-colors">
                         <td className="p-8">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                  ...
                                  <p className="text-[10px] lg:text-[12px] opacity-40  font-semibold ">{new Date(log.timestamp).toLocaleDateString()}</p>
                               </div>
                            </div>
                         </td>
                         <td className="p-8">
                         ...
                         <td className="p-8">
                            <div>
                               <p className="text-[11px] lg:text-[12px]  font-semibold tracking-tight mb-1">{log.subject}</p>
                               <span className={`px-2 py-0.5 rounded-md text-[11px] lg:text-[12px]  font-semibold tracking-tight border ${
                            ...
                            <div className="flex items-center gap-2">
                               {log.status === 'sent' ? (
                                  <>
                                    ...
                                    <span className="text-[11px] lg:text-[12px]  font-semibold tracking-tight text-emerald-500">Delivered</span>
                                  </>
                               ) : (
                                  <>
                                    <XCircle className="size-4 text-red-500" />
                                    <span className="text-[11px] lg:text-[12px]  font-semibold tracking-tight text-red-500">Failed</span>
                                  </>
                               )}
                            </div>
                         ...
                         <td className="p-8 text-right">
                            <button className="px-6 py-2 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[11px] lg:text-[12px]  font-semibold tracking-tight hover:bg-[var(--accent)] hover:text-white hover:border-transparent transition-all">
                               View Trace
                            </button>
                         </td>
                      </tr>
           ...
           <div className="p-8 border-t border-[var(--glass-border)] bg-[var(--bg-secondary)]/20 flex items-center justify-between">
              <p className="text-[11px] lg:text-[12px]  font-semibold opacity-40 tracking-tight">
