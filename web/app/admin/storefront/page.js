        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 animate-in fade-in slide-in-from-top-10 duration-700">
           <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                 ...
                 <div className="flex items-center gap-3">
                    <p className="text-[11px] lg:text-[12px] font-bold  tracking-[0.3em] text-[var(--text-secondary)] opacity-40">System Topology Control</p>
                    <div className="h-1 w-1 rounded-full bg-[var(--glass-border)]" />
                    <span className="text-[11px] lg:text-[12px] font-bold text-[var(--accent)] tracking-tight">{sections.length} Active Nodes</span>
                 </div>
              </div>
           </div>
           ...
           <div className="flex items-center gap-4">
              <button 
                ...
                className="h-14 px-8 rounded-2xl bg-[var(--text-primary)] text-[var(--bg-primary)] text-[11px] lg:text-[12px] font-bold  tracking-[0.2em] flex items-center gap-4 hover:scale-[1.03] active:scale-95 shadow-2xl transition-all group"
              >
           ...
           {loading ? (
             [...Array(3)].map((_, i) => (
              ...
              <div className="py-40 text-center glass-panel rounded-[3rem] border border-[var(--glass-border)] space-y-6">
                 ...
                 <div className="space-y-1">
                    <h3 className="text-2xl font-bold opacity-30 tracking-tight">No Sectors Synchronized</h3>
                      ...
                      <div className="flex lg:flex-col items-center lg:items-start gap-4 lg:gap-6 w-full">
                         ...
                         <div className="flex-1 lg:w-full space-y-1">
                            <div className="flex items-center gap-2">
                               <span className="text-[11px] lg:text-[12px] font-bold  tracking-[0.2em] bg-[var(--accent)] text-white px-2 py-0.5 rounded shadow-lg shadow-[var(--accent)]/20">
                                  {section.type.replace('_', ' ')}
                               </span>
                               <span className={`text-[11px] lg:text-[12px] font-bold px-2 py-0.5 rounded border ${section.is_active ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-white/5 text-white/40 border-white/10'} tracking-tight`}>
                            ...
                            <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-40  tracking-[0.2em]">{section.data?.length || 0} Linked Elements</p>
                         </div>
                      </div>
                      ...
                      <div className="flex lg:w-full items-center justify-between gap-4">
                         ...
                         <div className="flex items-center gap-2">
                            ...
                            <button 
                              ...
                              className="h-10 px-4 rounded-xl border border-[var(--glass-border)] bg-[var(--text-primary)] text-[var(--bg-primary)] hover:scale-[1.05] transition-all shadow-lg flex items-center gap-2 text-[11px] lg:text-[12px] font-bold tracking-tight"
                            >
                      ...
                      <div className="flex items-center gap-6 pb-2 min-w-full lg:min-w-0">
                         {section.data && section.data.length > 0 ? (
                                 ...
                                 <div key={idx} className="w-[180px] md:w-[220px] shrink-0 space-y-4 group/preview relative">
                                    <div className="aspect-[16/10] rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] overflow-hidden shadow-lg group-hover/preview:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-500 relative">
                                       {item.image_url ? (
                                         ...
                                         <div className="size-full flex flex-col items-center justify-center gap-2 opacity-20 bg-gradient-to-br from-[var(--glass-border)] to-transparent">
                                            ...
                                            <span className="text-[11px] lg:text-[12px] font-bold  tracking-[0.2em]">Category Mapping</span>
                                         </div>
                                       ) : (
                                       ...
                                       <div className="absolute bottom-3 left-3 right-3 px-3 py-2 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-between translate-y-4 opacity-0 group-hover/preview:translate-y-0 group-hover/preview:opacity-100 transition-all duration-300">
                                          <span className="text-[11px] lg:text-[12px] font-bold text-white tracking-tight truncate max-w-[100px]">{item.headline || item.category_name || item.product_name || vendorName || 'NODE'}</span>
                                    ...
                                    <div className="px-1 space-y-1">
                                       <div className="flex items-center justify-between">
                                          <h4 className="text-[11px] lg:text-[12px] font-bold text-[var(--text-primary)] tracking-tight truncate max-w-[140px]">{item.headline || item.category_name || item.product_name || vendorName || 'Unnamed Element'}</h4>
                                          {item.tag && <span className="text-[11px] lg:text-[12px] font-bold text-[var(--accent)]">{item.tag}</span>}
                                       </div>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-10 opacity-20 space-y-3">
                               ...
                               <p className="text-[11px] lg:text-[12px] font-bold  tracking-[0.3em]">No Dynamic Objects Linked</p>
                            </div>
                         )}
                      </div>
        ...
        {!loading && sections.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pt-16 border-t border-[var(--glass-border)] animate-in fade-in slide-in-from-bottom-10 duration-1000">
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-40  tracking-[0.3em]">Total Coverage</span>
             ...
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-bold text-emerald-500 opacity-40  tracking-[0.3em]">Live Manifest</span>
             ...
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-bold text-rose-500 opacity-40  tracking-[0.3em]">Inactive Nodes</span>
             ...
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-bold text-blue-500 opacity-40  tracking-[0.3em]">Complexity Score</span>
                <span className="text-4xl font-bold text-blue-500 tracking-tighter">{sections.reduce((acc, s) => acc + (s.data?.length || 0), 0)}</span>
