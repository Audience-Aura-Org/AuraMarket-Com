        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 animate-in fade-in slide-in-from-top-10 duration-700">
           <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                 <h1 className="text-4xl font-quicksand font-bold text-[var(--text-primary)] tracking-tighter leading-none">CMS <span className="text-[var(--accent)]">Architect</span></h1>
                 <div className="flex items-center gap-3">
                    <p className="text-[11px] lg:text-[12px] font-quicksand font-bold  tracking-[0.3em] text-[var(--text-secondary)] opacity-40">Homepage Topology Control</p>
           ...
           {loading ? (
             [...Array(3)].map((_, i) => (
              ...
              <div className="py-40 text-center glass-panel rounded-[3rem] border border-[var(--glass-border)] space-y-6">
                 ...
                 <div className="space-y-1">
                    <h3 className="text-2xl font-quicksand font-bold opacity-30 tracking-tight">No Components Manifested</h3>
                   ...
                   <div className="p-6 md:p-8 lg:w-80 border-b lg:border-b-0 lg:border-r border-[var(--glass-border)] bg-white/5 flex lg:flex-col items-center justify-between gap-6 shrink-0 backdrop-blur-md">
                      <div className="flex lg:flex-col items-center lg:items-start gap-4 lg:gap-6 w-full">
                         ...
                         <div className="flex-1 lg:w-full space-y-1">
                            ...
                            <div className="flex items-center gap-2 text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40  tracking-[0.2em]">
                      ...
                      <div className="flex items-center gap-6 pb-2 min-w-full lg:min-w-0">
                         {section.data && section.data.length > 0 ? (
                                 ...
                                 <div key={idx} className="w-[180px] md:w-[220px] shrink-0 space-y-4 group/preview relative">
                                    ...
                                    <div className="px-1 space-y-1">
                                       <div className="flex items-center justify-between">
                                          <h4 className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] tracking-tight truncate max-w-[140px]">{item.headline || item.category_name || item.product_name || vendorName || 'Unnamed Node'}</h4>
                                       </div>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-10 opacity-20 space-y-3">
                               ...
                               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold  tracking-[0.3em]">Module Devoid of Content</p>
                            </div>
                         )}
                      </div>
        ...
        {!loading && sections.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pt-16 border-t border-[var(--glass-border)] animate-in fade-in slide-in-from-bottom-10 duration-1000">
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40  tracking-[0.3em]">Total Components</span>
                <span className="text-4xl font-quicksand font-bold text-[var(--text-primary)] tracking-tighter">{sections.length}</span>
             </div>
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-quicksand font-bold text-emerald-500 opacity-40  tracking-[0.3em]">Live Components</span>
             ...
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-quicksand font-bold text-blue-500 opacity-40  tracking-[0.3em]">Scheduled Tasks</span>
                <span className="text-4xl font-quicksand font-bold text-blue-500 tracking-tighter">{sections.filter(s => s.scheduled_start).length}</span>
             </div>
             <div className="glass-panel p-8 rounded-[2rem] border border-[var(--glass-border)] flex flex-col items-center text-center space-y-2">
                <span className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--accent)] opacity-40  tracking-[0.3em]">Complexity Score</span>
                <span className="text-4xl font-quicksand font-bold text-[var(--accent)] tracking-tighter">{sections.reduce((acc, s) => acc + (s.data?.length || 0), 0)}</span>
             </div>
          </div>
        )}
