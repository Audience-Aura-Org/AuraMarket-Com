          ...
          <div className="flex items-center gap-4">
            ...
            {editing?._id === cat._id ? (
              <input 
                ...
                className="bg-[var(--bg-primary)] border border-[var(--accent)] rounded-xl px-4 py-2 text-[11px] lg:text-[12px] font-bold text-[var(--accent)] outline-none ring-4 ring-[var(--accent)]/5 shadow-inner capitalize tracking-tight"
                autoFocus
                onKeyDown={(e) => e.key === 'Enter' && handleUpdate()}
              />
            ) : (
              <div className="flex flex-col">
                <span className="text-[11px] lg:text-[12px] font-bold tracking-tight text-[var(--text-primary)] group-hover:text-[var(--accent)] transition-colors capitalize">{cat.name}</span>
                <span className="text-[10px] lg:text-[12px] font-bold tracking-widest text-[var(--text-secondary)] opacity-30 capitalize">Node_ID: #{cat._id?.slice(-8).toUpperCase()}</span>
              </div>
            )}
          </div>
      ...
      <header className="h-24 flex items-center justify-between px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl sticky top-16 z-40">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] tracking-tight opacity-50 capitalize">Operational Hierarchy // Node_Taxonomy_Root</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
           <button 
             ...
             className="h-11 px-8 bg-[var(--accent)] text-white rounded-2xl text-[10px] lg:text-[12px] font-bold tracking-[0.2em] capitalize shadow-lg shadow-[var(--accent)]/20 active:scale-95 transition-all flex items-center gap-2"
           >
      ...
      <div className="p-10 space-y-8 pb-40">
                     ...
                     <div className="relative z-10">
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                           <div className="flex-1 space-y-2">
                              <label className="text-[10px] lg:text-[12px] font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Node Descriptor</label>
                              <input 
                                 placeholder="DESIGNATE LABEL..."
                                 value={formData.name}
                                 onChange={e => setFormData({...formData, name: e.target.value})}
                                 className="w-full h-14 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl px-6 text-[11px] lg:text-[12px] font-bold capitalize outline-none focus:border-[var(--accent)] transition-all shadow-inner tracking-widest"
                                 autoFocus
                              />
                           </div>
                           <div className="flex items-end gap-3">
                              <button onClick={handleAdd} className="h-14 px-10 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-2xl font-bold text-[10px] lg:text-[12px] tracking-[0.3em] capitalize hover:bg-[var(--accent)] hover:text-white transition-all shadow-xl active:scale-95">Commit</button>
                              <button onClick={() => setIsAdding(false)} className="h-14 px-10 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl font-bold text-[10px] lg:text-[12px] tracking-[0.3em] capitalize hover:bg-rose-500 hover:text-white transition-all shadow-sm active:scale-95">Abort</button>
                           </div>
                        </div>
                     </div>
                  </div>
         ...
         <div className="glass-panel rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               ...
               <p className="text-[10px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-40 capitalize tracking-widest">Global Hierarchy Tree</p>
            </div>
