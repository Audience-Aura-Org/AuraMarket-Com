        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] tracking-tight  opacity-50">Monitoring immutable event stream</p>
            </div>
          </div>
        </div>
           ...
           <div className="flex items-center gap-2 bg-[var(--bg-secondary)]/50 p-1 rounded-xl border border-[var(--glass-border)]">
              {['all', 'product', 'user', 'dispute', 'transaction'].map(type => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-lg text-[11px] lg:text-[12px] font-bold tracking-tight transition-all ${activeFilter === type ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-[var(--text-secondary)] hover:bg-[var(--glass-border)]'}`}
                >
                  {type}
                </button>
              ))}
      ...
      <div className="p-10 space-y-8 pb-32">
         ...
         <div className="grid grid-cols-4 gap-4">
            {[
               ...
               <div key={s.label} className="glass-panel p-6 rounded-[2rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 hover:-translate-y-1 transition-all">
                  <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)]  tracking-[0.2em] mb-2 opacity-40">{s.label}</p>
         ...
         <div className="glass-panel rounded-[2.5rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               ...
               <div className="flex items-center gap-2 text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-50 tracking-tight">
            ...
            <div className="divide-y divide-[var(--glass-border)]/50">
               {loading ? (
                  <div className="py-40 flex flex-col items-center justify-center opacity-30">
                     ...
                     <p className="text-[11px] lg:text-[12px] font-bold tracking-tight">Decrypting Mutation Stream...</p>
                  </div>
                        ...
                        <div className="flex-1 min-w-0">
                           <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-3">
                                 ...
                                 <span className="px-3 py-1 rounded-lg bg-[var(--bg-secondary)] text-[var(--text-secondary)] text-[11px] lg:text-[12px] font-bold tracking-tight border border-[var(--glass-border)]">
                                    {log.user_id?.role || 'SYSTEM'}
                                 </span>
                                 <span className={`px-3 py-1 rounded-lg text-[11px] lg:text-[12px] font-bold tracking-tight ${getActionColor(log.action)}`}>
                                    {log.action.replace('_', ' ')}
                                 </span>
                              </div>
                              <time className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)] opacity-40 tracking-tight flex items-center gap-2">
                           ...
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                              <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-5 rounded-2xl">
                                 <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)]  tracking-[0.2em] mb-3 opacity-50 flex items-center gap-2">
                                    <FileText className="w-3 h-3" /> Targeted Entity
                                 </p>
                                 <div className="flex items-center justify-between">
                                    <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-primary)] flex items-center gap-2">
                              ...
                              <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-5 rounded-2xl overflow-hidden">
                                 <p className="text-[11px] lg:text-[12px] font-bold text-[var(--text-secondary)]  tracking-[0.2em] mb-3 opacity-50">Mutation Payload</p>
