    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      <header className="h-24 flex items-center justify-between px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] shrink-0 sticky top-16 z-10 backdrop-blur-xl text-[var(--text-primary)]">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-black text-[var(--text-primary)] tracking-tight ">System <span className="text-[var(--accent)]">Audit</span> Ledger</h2>
            <div className="flex items-center gap-2 mt-1">
               <div className="size-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse" />
               <p className="text-[10px] font-black text-[var(--text-secondary)] tracking-widest  opacity-50">Monitoring immutable event stream</p>
            </div>
          </div>
        </div>
           ...
           <div className="flex items-center gap-2 bg-[var(--bg-secondary)]/50 p-1 rounded-xl border border-[var(--glass-border)]">
              {['all', 'product', 'user', 'dispute', 'transaction'].map(type => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-lg text-[9px] font-black tracking-wide transition-all ${activeFilter === type ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-[var(--text-secondary)] hover:bg-[var(--glass-border)]'}`}
                >
                  {type}
                </button>
              ))}
      ...
      <div className="p-10 space-y-8 pb-32">
         ...
         <div className="grid grid-cols-4 gap-4">
            {[
               ...
               <div key={s.label} className="glass-panel p-6 rounded-[2rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 hover:-translate-y-1 transition-all">
                  <p className="text-[9px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-2 opacity-40">{s.label}</p>
         ...
         <div className="glass-panel rounded-[2.5rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               <h3 className="text-sm font-black text-[var(--text-primary)] tracking-wide flex items-center gap-3">
                  <Clock className="w-4 h-4 text-[var(--accent)]" /> 
                  Chronological Mutation Stream
               </h3>
               <div className="flex items-center gap-2 text-[10px] font-bold text-[var(--text-secondary)] opacity-50 tracking-wide">
            ...
            <div className="divide-y divide-[var(--glass-border)]/50">
               {loading ? (
                  <div className="py-40 flex flex-col items-center justify-center opacity-30">
                     ...
                     <p className="text-[10px] font-black tracking-wide">Decrypting Mutation Stream...</p>
                  </div>
                        ...
                        <div className="flex-1 min-w-0">
                           <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-3">
                                 <span className="text-sm font-black text-[var(--text-primary)] tracking-tight">{log.user_id?.name || 'System Operator'}</span>
                                 <span className="px-3 py-1 rounded-lg bg-[var(--bg-secondary)] text-[var(--text-secondary)] text-[8px] font-black tracking-wide border border-[var(--glass-border)]">
                                    {log.user_id?.role || 'SYSTEM'}
                                 </span>
                                 <span className={`px-3 py-1 rounded-lg text-[8px] font-black tracking-wide ${getActionColor(log.action)}`}>
                                    {log.action.replace('_', ' ')}
                                 </span>
                              </div>
                              <time className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 tracking-wide flex items-center gap-2">
                           ...
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                              <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-5 rounded-2xl">
                                 <p className="text-[8px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-3 opacity-50 flex items-center gap-2">
                              ...
                              <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-5 rounded-2xl overflow-hidden">
                                 <p className="text-[8px] font-black text-[var(--text-secondary)]  tracking-[0.2em] mb-3 opacity-50">Mutation Payload</p>
                  ...
                  <div className="py-40 flex flex-col items-center justify-center opacity-20 px-10 text-center">
                     ...
                     <p className="text-sm font-black  tracking-[0.2em] leading-relaxed max-w-sm">No system mutations recorded in this vector.</p>
                  </div>
               )}
            </div>
