    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <header className="h-24 flex items-center justify-between px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl sticky top-16 z-40">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-quicksand font-bold text-[var(--text-primary)] tracking-tight ">Transit <span className="text-[var(--accent)]">Protocol</span> Monitoring</h2>
            <div className="flex items-center gap-2 mt-1">
               <div className="size-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse" />
               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight opacity-50 capitalize">Operational Pipeline // Node_Transit_Master</p>
            </div>
          </div>
        </div>
      ...
      <div className="p-10 space-y-8 pb-40">
         ...
         <div className="glass-panel rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               ...
               <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40 capitalize tracking-widest">Global Fulfillment Synchronized</p>
            </div>
                          ...
                          {activeTab === 'Shipments' ? shipments.map(s => (
                             <tr key={s._id} className="group hover:bg-[var(--accent)]/5 transition-all">
                                <td className="px-10 py-6">
                                   <div className="flex items-center gap-4">
                                      ...
                                      <div className="min-w-0">
                                         <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] font-mono tracking-tight capitalize">#{s.tracking_code || s._id.slice(-8).toUpperCase()}</p>
                                         <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40 mt-1 flex items-center gap-1.5 capitalize">
                                ...
                                <td className="px-6 py-6">
                                   <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] capitalize">{s.logistics_id?.company_name || 'PENDING_CARRIER'}</p>
                                   <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-30 mt-1 capitalize">From: {s.vendor_id?.store_name || 'MERCHANT'}</p>
                                </td>
                                <td className="px-6 py-6 font-mono text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)]">
                                   {(s.price || 0).toLocaleString()} <span className="opacity-30">XAF</span>
                                </td>
                                <td className="px-6 py-6">
                                   <span className={`px-3 py-1 rounded-full text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest border capitalize ${
                             ...
                             <tr key={f._id} className="group hover:bg-[var(--accent)]/5 transition-all">
                                <td className="px-10 py-6">
                                   <div className="flex items-center gap-4">
                                      <div className="size-11 rounded-xl bg-indigo-500/10 text-indigo-500 flex items-center justify-center font-quicksand font-bold border border-indigo-500/10 overflow-hidden shadow-sm">
                                         {f.user_id?.branding?.logo ? <img src={f.user_id.branding.logo} className="size-full object-cover" /> : f.company_name[0].toUpperCase()}
                                      </div>
                                      <div className="min-w-0">
                                         <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] capitalize tracking-tight">{f.company_name}</p>
                                         <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-30 mt-1 capitalize truncate max-w-[200px]">{f.user_id?.email}</p>
                                      </div>
                                   </div>
                                </td>
                                <td className="px-6 py-6">
                                   <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                                      {f.service_regions?.map(r => (
                                         <span key={r} className="px-2 py-0.5 rounded-lg bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] capitalize tracking-tighter">{r}</span>
                                      ))}
                                   </div>
                                </td>
                                <td className="px-6 py-6">
                                   <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40 capitalize tracking-widest">
                                      {f.vehicle_types?.join(' // ')}
                                   </p>
                                </td>
                                <td className="px-6 py-6">
                                   <span className={`px-3 py-1 rounded-full text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest border capitalize ${f.is_verified ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                                ...
                                <td className="px-10 py-6 text-right">
                                   <div className="flex items-center justify-end gap-2">
                                      ...
                                      <button 
                                         ...
                                         className={`px-4 py-2 rounded-xl text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest capitalize transition-all shadow-sm ${f.is_verified ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20 hover:bg-rose-500 hover:text-white' : 'bg-[var(--accent)] text-white hover:opacity-90'}`}
                                      >
                             ...
                             <tr key={z._id} className="group hover:bg-[var(--accent)]/5 transition-all">
                                <td className="px-10 py-6 text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] font-mono capitalize tracking-tight">{z.name}</td>
                                <td className="px-6 py-6">
                                   <span className="px-3 py-1 rounded-lg bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] capitalize tracking-widest">{z.type}</span>
                                </td>
                                <td className="px-6 py-6 text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-30 capitalize tracking-widest">{z.parent_id?.name || 'ROOT_CLUSTER'}</td>
                                <td className="px-6 py-6">
                                   <div className="flex items-center gap-2">
                                      <div className="size-1.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_#10b981]" />
                                      <span className="text-[10px] lg:text-[12px] font-quicksand font-bold text-emerald-500 capitalize tracking-widest">Active Node</span>
                                   </div>
                                </td>
                                <td className="px-10 py-6 text-right">
                                   <button className="px-4 py-2 rounded-xl border border-rose-500/20 text-rose-500 text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest capitalize hover:bg-rose-500 hover:text-white transition-all">Purge</button>
                                </td>
                             </tr>
                          ))}
                       </tbody>
               ...
               <div className="relative z-10">
                  ...
                  <form className="space-y-6" onSubmit={async (e) => {
                     ...
                     <div className="space-y-2">
                        <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Zone Identifier</label>
                        <input 
                           placeholder="NODE_NAME (E.G. AKWA)"
                           value={newZone.name}
                           onChange={e => setNewZone({...newZone, name: e.target.value})}
                           className="w-full h-14 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl px-5 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none focus:border-[var(--accent)] transition-all shadow-inner"
                        />
                     </div>
                     <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                           <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Topology Type</label>
                           <select 
                              value={newZone.type}
                              onChange={e => setNewZone({...newZone, type: e.target.value, parent_id: ''})}
                              className="w-full h-14 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl px-5 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none cursor-pointer focus:border-[var(--accent)]"
                           >
                        ...
                        {newZone.type === 'quartier' && (
                           <div className="space-y-2">
                              <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Parent Cluster</label>
                              <select 
                     ...
                     <button className="w-full h-14 bg-[var(--accent)] text-white rounded-2xl font-quicksand font-bold text-[10px] lg:text-[12px] tracking-[0.3em] capitalize shadow-lg shadow-[var(--accent)]/20 hover:scale-[1.02] active:scale-95 transition-all">
                        Sync Node to Matrix
                     </button>
                  </form>
               </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-5xl w-full glass-panel rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/90 backdrop-blur-3xl p-12 relative z-10 flex flex-col max-h-[90vh] shadow-2xl">
             <div className="flex items-start justify-between mb-12 shrink-0">
                <div className="flex items-center gap-6">
                   <div className="size-16 rounded-[1.5rem] bg-[var(--accent)]/10 text-[var(--accent)] flex items-center justify-center font-quicksand font-bold border border-[var(--accent)]/20 text-2xl shadow-lg relative overflow-hidden">
                   ...
                   <div>
                      <h3 className="text-3xl font-quicksand font-bold tracking-tighter">{selectedFirm.company_name}</h3>
                      <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--accent)] tracking-[0.4em] capitalize opacity-60">Tariff Calibration Matrix</p>
                   </div>
                </div>
                <button onClick={() => setSelectedFirm(null)} className="size-12 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all text-[11px] lg:text-[12px] font-quicksand font-bold tracking-widest shadow-sm">
                   ESC
                </button>
             </div>
             ...
             <div className="flex-1 overflow-y-auto no-scrollbar pr-4">
                <div className="grid lg:grid-cols-2 gap-12">
                   <div className="space-y-8">
                      <h4 className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-[0.3em] capitalize opacity-40 border-l-2 border-[var(--accent)] pl-4">Inject Price Node</h4>
                      <div className="space-y-5">
                         <div className="space-y-2">
                            <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Topology Node</label>
                            <select 
                               value={priceEditor.quartier}
                               onChange={e => setPriceEditor({...priceEditor, quartier: e.target.value})}
                               className="w-full h-14 bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] rounded-2xl px-5 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none focus:border-[var(--accent)]"
                            >
                         ...
                         <div className="space-y-2">
                            <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest text-[var(--text-secondary)] opacity-40 capitalize ml-1">Operational Tariff (XAF)</label>
                            <div className="relative">
                               <input 
                                  ...
                                  className="w-full h-14 bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] rounded-2xl px-14 text-[11px] lg:text-[12px] font-quicksand font-bold font-mono outline-none focus:border-[var(--accent)]"
                               />
                         ...
                         <button 
                            ...
                            className="w-full h-14 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-2xl font-quicksand font-bold text-[10px] lg:text-[12px] tracking-[0.3em] capitalize shadow-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30"
                         >
                            Commit Tariff Node
                         </button>
                      </div>
                   ...
                   <div className="space-y-8">
                      <h4 className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-[0.3em] capitalize opacity-40 border-l-2 border-emerald-500 pl-4">Active Tariff Grid</h4>
                      <div className="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto no-scrollbar pb-10">
                         {(selectedFirm.quartier_prices || []).map(p => (
                            <div key={p.quartier} className="flex justify-between items-center p-5 rounded-2xl bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)]/50 hover:border-[var(--accent)]/40 transition-all group/item shadow-sm">
                               <span className="text-[11px] lg:text-[12px] font-quicksand font-bold capitalize tracking-tight text-[var(--text-primary)]">{p.quartier}</span>
                               <span className="text-sm font-mono font-quicksand font-bold text-[var(--accent)] group-hover/item:scale-110 transition-transform">{p.price.toLocaleString()} <span className="text-[10px] lg:text-[12px] opacity-30 ml-1">XAF</span></span>
                            </div>
                         ))}
                         {(!selectedFirm.quartier_prices || selectedFirm.quartier_prices.length === 0) && (
                            <div className="py-20 flex flex-col items-center justify-center gap-4 opacity-10 border-2 border-dashed border-[var(--glass-border)] rounded-[3rem]">
                               ...
                               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold capitalize tracking-[0.3em]">Grid Void</p>
                            </div>
                         )}
                      </div>
                   </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6">
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative z-10 w-full max-w-2xl rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)] p-10 shadow-2xl">
            <div className="mb-8 flex items-center justify-between">
               <div>
                  <h3 className="text-xl font-quicksand font-bold tracking-tighter">Fulfillment Override</h3>
                  <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40 capitalize tracking-widest mt-1">Manual Package Calibration</p>
               </div>
               <button onClick={() => setSelectedShipment(null)} className="size-10 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight">ESC</button>
            </div>
            
            <div className="grid grid-cols-2 gap-6">
               <div className="space-y-2">
                  <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest opacity-30 capitalize ml-1">Transit ID</label>
                  <input value={shipmentEdit.tracking_code} onChange={(e) => setShipmentEdit((s) => ({ ...s, tracking_code: e.target.value }))} className="w-full h-12 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none focus:border-[var(--accent)]" />
               </div>
               <div className="space-y-2">
                  <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest opacity-30 capitalize ml-1">Manifest Tariff</label>
                  <input type="number" value={shipmentEdit.price} onChange={(e) => setShipmentEdit((s) => ({ ...s, price: e.target.value }))} className="w-full h-12 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[11px] lg:text-[12px] font-quicksand font-bold font-mono outline-none focus:border-[var(--accent)]" />
               </div>
               <div className="space-y-2">
                  <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest opacity-30 capitalize ml-1">Network State</label>
                  <select value={shipmentEdit.status} onChange={(e) => setShipmentEdit((s) => ({ ...s, status: e.target.value }))} className="w-full h-12 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none cursor-pointer">
               ...
               <div className="space-y-2">
                  <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest opacity-30 capitalize ml-1">Assigned Carrier</label>
                  <select value={shipmentEdit.logistics_id} onChange={(e) => setShipmentEdit((s) => ({ ...s, logistics_id: e.target.value }))} className="w-full h-12 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[11px] lg:text-[12px] font-quicksand font-bold capitalize outline-none cursor-pointer">
            ...
            <div className="mt-6 space-y-2">
               <label className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest opacity-30 capitalize ml-1">Internal Log</label>
               <textarea rows={3} value={shipmentEdit.note} onChange={(e) => setShipmentEdit((s) => ({ ...s, note: e.target.value }))} className="w-full bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl px-4 py-3 text-[11px] lg:text-[12px] font-quicksand font-bold outline-none focus:border-[var(--accent)]" />
            </div>

            <button onClick={saveShipmentEdit} className="mt-8 w-full h-14 bg-[var(--accent)] text-white rounded-2xl font-quicksand font-bold text-[10px] lg:text-[12px] tracking-[0.3em] capitalize shadow-lg shadow-[var(--accent)]/20 hover:scale-[1.02] active:scale-95 transition-all">
