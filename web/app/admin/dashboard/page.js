  ...
  const content = (
    <div className="bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-2xl p-4 hover:border-[var(--accent)]/30 transition-all group cursor-pointer">
      <div className="flex items-center gap-3 mb-3">
        ...
        <p className="text-[10px] font-bold text-[var(--text-secondary)] tracking-tight opacity-50">{title}</p>
      </div>
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 tracking-tight">Global Administrative Hub</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/5 border border-emerald-500/10">
                <div className="size-1.5 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[8px] font-bold text-emerald-500 ">Systems Nominal</span>
             </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-xl bg-[var(--bg-secondary)]/20 border border-[var(--glass-border)] text-center group hover:bg-[var(--bg-primary)] transition-all">
                      ...
                      <p className="text-[8px] font-bold tracking-tight text-[var(--text-secondary)] opacity-40">{item.label}</p>
                    </div>
                  ))}
                </div>
             </div>
          ...
          <section className="lg:col-span-1 space-y-6">
             <div className="bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-[2.5rem] p-8 shadow-sm">
                ...
                <div className="space-y-4">
                   <div className="p-5 rounded-2xl bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] group hover:border-[var(--accent)]/30 transition-all">
                      ...
                      <Link href="/admin/security" className="inline-flex items-center gap-2 text-[9px] font-bold text-[var(--accent)] tracking-tight hover:gap-4 transition-all">
             ...
             <div className="bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-[2.5rem] p-8 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <Link href="/admin/logs" className="text-[8px] font-bold text-[var(--accent)] tracking-tight hover:underline">View All</Link>
                </div>
                <div className="space-y-6">
                   {[
                     ...
                     <div key={i} className="flex gap-4 group">
                        ...
                        <div>
                          <p className="text-[10px] font-bold tracking-tight">{log.title}</p>
                          <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 mt-0.5">{log.desc}</p>
                          <span className="text-[8px] font-bold text-[var(--text-secondary)] opacity-20 mt-2 block tracking-tight">{log.time}</span>
                        </div>
                     </div>
                   ))}
                </div>
