    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <header className="h-24 flex items-center justify-between px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl sticky top-16 z-40">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-xl font-quicksand font-bold text-[var(--text-primary)] tracking-tight ">Order <span className="text-[var(--accent)]">Manifest</span> Ledger</h2>
            <div className="flex items-center gap-2 mt-1">
               <div className="size-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse" />
               <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight opacity-50 capitalize">Operational Pipeline // Node_Order_Control</p>
            </div>
          </div>
        </div>
      ...
      <div className="p-10 space-y-8 pb-40">
         ...
         <div className="glass-panel rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               ...
               <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-40 capitalize tracking-widest">Global Synchronization Active</p>
            </div>
            ...
            <div className="space-y-4">
              {loading ? (
                           ...
                           >
                              ...
                              <div className="text-right shrink-0">
                                 ...
                                 <div className="flex items-center justify-end gap-3 mt-2">
                                    <span className={`px-2 py-0.5 rounded text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest capitalize border ${payment.bg} ${payment.color} ${payment.color.replace('text-', 'border-')}/20`}>
                                       {payment.label}
                                    </span>
                                 ...
                                 >
                                    <div className="px-8 pb-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
                                       ...
                                       <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-6 rounded-3xl space-y-4">
                                          <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-[0.2em] mb-2 opacity-50 capitalize flex items-center gap-2">
                                             <Database className="w-3 h-3" /> Entity Metadata
                                          </p>
                                          <div className="grid grid-cols-2 gap-4">
                                             <div>
                                                <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-30 capitalize tracking-widest mb-1">Customer</p>
                                                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)]">{customer?.name}</p>
                                                <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-50">{customer?.email}</p>
                                             </div>
                                             <div>
                                                <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-30 capitalize tracking-widest mb-1">Store Node</p>
                                                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--accent)]">{order.vendor_id?.store_name}</p>
                                             </div>
                                             <div className="col-span-2">
                                                <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-30 capitalize tracking-widest mb-1">Shipping Terminal</p>
                                                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)]">{order.shipping_address?.quartier}, {order.shipping_address?.address}</p>
                                             </div>
                                          </div>
                                       </div>
                                          ...
                                          <div className="space-y-2 max-h-48 overflow-y-auto no-scrollbar">
                                             {order.products?.map((it, idx) => (
                                                <div key={idx} className="flex items-center gap-3 p-2 rounded-xl bg-[var(--bg-primary)]/50 border border-[var(--glass-border)]">
                                                   ...
                                                   <div className="flex-1 min-w-0">
                                                      <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-primary)] truncate">{it.name}</p>
                                                      <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-50">x{it.quantity} @ {it.price?.toLocaleString()} XAF</p>
                                                   </div>
                                                </div>
                                             ))}
                                          </div>
                                       ...
                                       <div className="bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] p-6 rounded-3xl space-y-4">
                                          ...
                                          <div className="space-y-3">
                                             <select
                                                ...
                                                className="w-full h-11 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-xl px-4 text-[11px] lg:text-[12px] font-quicksand font-bold outline-none focus:border-[var(--accent)]"
                                             >
                                             ...
                                             <button
                                                ...
                                                className="w-full h-12 bg-[var(--accent)] text-white rounded-xl font-quicksand font-bold text-[10px] lg:text-[12px] tracking-widest capitalize shadow-lg shadow-[var(--accent)]/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                                             >
                 ...
                 <div className="py-40 flex flex-col items-center justify-center opacity-20 px-10 text-center">
                    ...
                    <p className="text-sm font-quicksand font-bold tracking-[0.2em] capitalize leading-relaxed max-w-sm">No operational manifests detected in this node.</p>
                 </div>
              )}
            </div>
