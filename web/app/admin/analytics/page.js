        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 tracking-tight">Global Platform Insight</p>
            </div>
          </div>
            ...
            <div className="flex bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl p-0.5">
              {['overview', 'revenue', 'inventory'].map(t => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-bold tracking-tight transition-all ${activeTab === t ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-[var(--text-secondary)] opacity-40 hover:opacity-100'}`}
                >
                  {t}
                </button>
              ))}
      ...
      <div className="p-6 md:p-8 space-y-8 max-w-[1600px] mx-auto pb-32">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 p-8 rounded-[2.5rem] bg-[var(--bg-secondary)]/20 border border-[var(--glass-border)] relative overflow-hidden">
             <div className="flex items-center justify-between mb-8 relative z-10">
                <div>
                   ...
                   <p className="text-[9px] font-bold text-[var(--accent)] tracking-tight mt-1">Real-Time Market Pulse</p>
                </div>
          ...
          <div className="p-8 rounded-[2.5rem] bg-[var(--bg-secondary)]/20 border border-[var(--glass-border)]">
             <h3 className="text-[10px] font-bold tracking-tight opacity-40 mb-8">
                ...
                {(activeTab === 'inventory' ? category_stats : role_breakdown)?.map((item, i) => (
                   <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-bold ">{item._id || 'Standard'}</span>
             ...
             <div className="mt-12 pt-8 border-t border-[var(--glass-border)]">
                <h4 className="text-[9px] font-bold tracking-tight opacity-20 mb-6">Inventory Health</h4>
                <div className="flex items-center justify-between p-4 rounded-2xl bg-red-500/5 border border-red-500/10">
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-[9px] font-bold ">Stock Alerts</span>
                   </div>
        ...
        <div className="p-8 rounded-[2.5rem] bg-[var(--bg-secondary)]/20 border border-[var(--glass-border)]">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-[10px] font-bold tracking-tight opacity-40">Asset Conversion Performance</h3>
              ...
              {top_products?.slice(0, 6).map((p, i) => (
                <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)]/30 transition-all group">
                   ...
                   <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[8px] font-bold text-[var(--accent)]  mt-0.5">XAF {fmt(p.price)}</p>
                   </div>
