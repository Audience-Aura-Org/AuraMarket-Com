        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <p className="text-[11px] font-bold text-[var(--text-secondary)] tracking-tight opacity-50 uppercase">Auth Queue Active // Node_Governance_Master</p>
            </div>
          </div>
        </div>
           ...
           <div className="flex bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl p-1">
              {['Vendors', 'Products'].map(tab => (
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold tracking-tight transition-all uppercase ${activeTab === tab ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
                >
                  {tab}
                </button>
              ))}
      ...
      <div className="p-10 space-y-8 pb-40">
         ...
         <div className="glass-panel rounded-[3rem] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 overflow-hidden shadow-2xl">
            <div className="p-8 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/30 flex items-center justify-between">
               ...
               <p className="text-[10px] font-bold text-[var(--text-secondary)] opacity-40 uppercase tracking-widest">Global Governance Active</p>
            </div>
            ...
            <div className="min-h-[400px]">
              {loading ? (
                 ...
                 <div className="py-40 flex flex-col items-center justify-center opacity-20 px-10 text-center">
                    ...
                    <p className="text-sm font-bold tracking-[0.2em] uppercase leading-relaxed max-w-sm">No pending nodes in the synchronization queue.</p>
                 </div>
              ) : (
                                ...
                                <td className="px-10 py-6">
                                   <div className="flex items-center gap-4">
                                      ...
                                      <div className="min-w-0">
                                         <p className="text-[11px] font-bold text-[var(--text-primary)] tracking-tight uppercase truncate max-w-[200px]">{v.vendor_id?.store_name || v.user_id?.name}</p>
                                         <p className="text-[10px] font-bold text-[var(--text-secondary)] opacity-40 mt-1 font-mono uppercase">#{v._id.slice(-6).toUpperCase()}</p>
                                      </div>
                                   </div>
                                </td>
                                <td className="px-6 py-6">
                                   <p className="text-[11px] font-bold text-[var(--text-primary)] uppercase">{v.user_id?.email}</p>
                                   <p className="text-[10px] font-bold text-[var(--text-secondary)] opacity-30 mt-1 uppercase flex items-center gap-1.5">
                             ...
                             <tr key={p._id} className="group hover:bg-[var(--accent)]/5 transition-all">
                                <td className="px-10 py-6">
                                   <div className="flex items-center gap-4">
                                      ...
                                      <div className="min-w-0">
                                         <p className="text-[11px] font-bold text-[var(--text-primary)] tracking-tight uppercase truncate max-w-[200px]">{p.name}</p>
                                         <p className="text-[10px] font-bold text-[var(--text-secondary)] opacity-40 mt-1 font-mono uppercase">#{p._id.slice(-6).toUpperCase()}</p>
                                      </div>
                                   </div>
                                </td>
                                <td className="px-6 py-6">
                                   <p className="text-[11px] font-bold text-[var(--text-primary)] uppercase">{p.vendor_id?.store_name}</p>
