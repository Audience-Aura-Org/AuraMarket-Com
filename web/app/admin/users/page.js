      ...
      <div className="px-6 py-6 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/10 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black  tracking-tighter">Identity Matrix</h1>
              <p className="text-[9px] font-black text-[var(--text-secondary)] opacity-40 tracking-wide">Global Personnel Directory</p>
            </div>
          </div>
            ...
            <div className="flex bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl p-0.5">
              {['all', 'customer', 'vendor', 'admin'].map(r => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black tracking-wide transition-all ${roleFilter === r ? 'bg-[var(--accent)] text-white' : 'text-[var(--text-secondary)] opacity-40'}`}
                >
                  {r}
                </button>
              ))}
           ...
           <div className="space-y-2 min-h-[600px]">
              {currentUsers.map(u => (
                <div key={u._id} className="flex items-center gap-4 p-3 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)]/30 transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <p className="text-[9px] font-black tracking-wide truncate">{u.name}</p>
                      <span className={`px-2 py-0.5 rounded-full text-[7px] font-black tracking-wide border ${u.role === 'admin' ? 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20' : u.role === 'vendor' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}>
                        {u.role}
                      </span>
                    </div>
                    <p className="text-[9px] font-black text-[var(--text-secondary)] opacity-40 truncate  mt-0.5 tracking-widest">{u.email}</p>
                  </div>
                  <div className="flex items-center gap-8">
                     <div className="text-right">
                        <div className="flex items-center gap-2 justify-end">
                           ...
                           <span className="text-[9px] font-black tracking-wide opacity-60">{u.verification_status || 'Pending'}</span>
                        </div>
                        <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-20  mt-0.5 tracking-widest">Node Status</p>
                     </div>
              ...
              {filteredUsers.length === 0 && (
                 <div className="py-20 text-center border border-dashed border-[var(--glass-border)] rounded-[2rem] opacity-20">
                    <p className="text-xs font-black tracking-wide">No identity nodes synchronized</p>
                 </div>
              )}
           </div>
        )}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-md bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-[2.5rem] p-8 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-black  tracking-tighter">Node Calibrator</h3>
                    <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-40 tracking-wide">Surgical Identity Override</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[9px] font-black tracking-wide opacity-40">Alias</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[9px] font-black tracking-wide opacity-40">Matrix Role</p>
                    <select value={editForm.role} onChange={e=>setEditForm(f=>({...f,role:e.target.value}))} className="w-full h-11 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[10px] font-black tracking-wide outline-none appearance-none cursor-pointer">
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black tracking-wide opacity-40">Identity Email</p>
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black tracking-wide opacity-40">Signal Phone</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[9px] font-black tracking-wide opacity-40">Node Status</p>
                    <select value={editForm.verification_status} onChange={e=>setEditForm(f=>({...f,verification_status:e.target.value}))} className="w-full h-11 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[10px] font-black tracking-wide outline-none appearance-none cursor-pointer">
                  ...
                  <div className="space-y-1">
                    <p className="text-[9px] font-black tracking-wide opacity-40">Cipher Override</p>
                ...
                <div className="pt-4">
                  <button type="submit" disabled={submitting} className="w-full h-14 bg-[var(--accent)] text-white rounded-2xl font-black text-[9px] tracking-wide shadow-lg shadow-[var(--accent)]/20 active:scale-95 transition-all">
                    {submitting ? (
