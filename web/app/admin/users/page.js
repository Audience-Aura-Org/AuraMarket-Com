        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 tracking-tight">Global Personnel Directory</p>
            </div>
          </div>
            ...
            <div className="flex bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl p-0.5">
              {['all', 'customer', 'vendor', 'admin'].map(r => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-bold tracking-tight transition-all ${roleFilter === r ? 'bg-[var(--accent)] text-white' : 'text-[var(--text-secondary)] opacity-40'}`}
                >
                  {r}
                </button>
              ))}
              ...
              {currentUsers.map(u => (
                <div key={u._id} className="flex items-center gap-4 p-3 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)]/30 transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <p className="text-[9px] font-bold tracking-tight truncate">{u.name}</p>
                    ...
                    <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 truncate  mt-0.5 tracking-tight">{u.email}</p>
                  </div>
                  <div className="flex items-center gap-8">
                     <div className="text-right">
                        <div className="flex items-center gap-2 justify-end">
                           ...
                           <span className="text-[9px] font-bold tracking-tight opacity-60">{u.verification_status || 'Pending'}</span>
                        </div>
                        <p className="text-[8px] font-bold text-[var(--text-secondary)] opacity-20  mt-0.5 tracking-tight">Node Status</p>
                     </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[8px] font-bold text-[var(--text-secondary)] opacity-40 tracking-tight">Surgical Identity Override</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[9px] font-bold tracking-tight opacity-40">Alias</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[9px] font-bold tracking-tight opacity-40">Matrix Role</p>
                    <select value={editForm.role} onChange={e=>setEditForm(f=>({...f,role:e.target.value}))} className="w-full h-11 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[10px] font-bold tracking-tight outline-none appearance-none cursor-pointer">
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-bold tracking-tight opacity-40">Identity Email</p>
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-bold tracking-tight opacity-40">Signal Phone</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[9px] font-bold tracking-tight opacity-40">Node Status</p>
                    <select value={editForm.verification_status} onChange={e=>setEditForm(f=>({...f,verification_status:e.target.value}))} className="w-full h-11 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl px-4 text-[10px] font-bold tracking-tight outline-none appearance-none cursor-pointer">
                  ...
                  <div className="space-y-1">
                    <p className="text-[9px] font-bold tracking-tight opacity-40">Cipher Override</p>
