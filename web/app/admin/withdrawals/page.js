...
function KPI({ title, value, icon: Icon, color, sub }) {
  ...
  return (
    <div className="bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-3xl p-6 relative overflow-hidden glass-panel">
      ...
      <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-[0.15em] text-[var(--text-secondary)] opacity-50">{title}</p>
      <h3 className="text-2xl font-quicksand font-bold text-[var(--text-primary)] mt-1 tabular-nums">{value}</h3>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }}
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-lg font-quicksand font-bold tracking-tight">Withdrawal Review</h2>
        ...
        <div className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] mb-6">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-quicksand font-bold text-[var(--text-primary)] truncate">{wr.requestedBy?.name || '—'}</p>
            <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] opacity-50 truncate">{wr.requestedBy?.email}</p>
            <span className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest px-2 py-0.5 rounded-full bg-[var(--accent)]/10 text-[var(--accent)]">{wr.role?.toUpperCase()}</span>
          </div>
          <div className="text-right shrink-0">
            <p className="text-xl font-quicksand font-bold tabular-nums">{fmt(wr.amount)}</p>
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-30">{wr.currency}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)]">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-40 mb-1">Method</p>
            <div className="flex items-center gap-2">
              <MIcon className="size-4 text-[var(--accent)]" />
              <p className="text-sm font-quicksand font-bold capitalize">{wr.withdrawalMethod}</p>
            </div>
          </div>
          <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)]">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-40 mb-1">Status</p>
            <div className={`flex items-center gap-1 text-xs font-quicksand font-bold ${S.cls.split(' ').slice(1).join(' ')}`}>
              <SIcon className="size-3" />
              <span className="capitalize">{wr.status}</span>
            </div>
          </div>
          <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)]">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-40 mb-1">Recipient</p>
            <p className="text-xs font-quicksand font-bold">{rd.firstName} {rd.lastName}</p>
          ...
          <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)]">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-40 mb-1">Destination</p>
            <p className="text-xs font-quicksand font-bold font-mono truncate">{rd.phoneNumber || rd.accountNumber || rd.eversendTag || '—'}</p>
        ...
        {wr.eversendTransactionId && (
          <div className="p-4 rounded-2xl bg-[var(--accent)]/5 border border-[var(--accent)]/20 mb-6 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-50">Eversend Transaction ID</p>
              <p className="text-xs font-mono font-quicksand font-bold truncate">{wr.eversendTransactionId}</p>
            </div>
        ...
        {(wr.rejectionReason || wr.failureReason) && (
          <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/20 mb-6">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold text-red-500 mb-1">{wr.rejectionReason ? 'Rejection Reason' : 'Failure Reason'}</p>
        ...
        {wr.note && (
          <div className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] mb-6">
            <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-40 mb-1">Requester Note</p>
        ...
        {wr.status === 'pending' && !showRejectForm && (
          <div className="flex gap-3">
            <button onClick={() => onApprove(wr._id)} disabled={!!processing}
              className="flex-1 h-13 bg-emerald-500 text-white rounded-2xl font-quicksand font-bold text-xs tracking-tight flex items-center justify-center gap-2 hover:brightness-110 disabled:opacity-50 transition-all shadow-lg shadow-emerald-500/20 py-3">
            ...
            <button onClick={() => setShowRejectForm(true)} disabled={!!processing}
              className="flex-1 h-13 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl font-quicksand font-bold text-xs tracking-tight flex items-center justify-center gap-2 hover:bg-red-500/20 disabled:opacity-50 transition-all py-3">
        ...
        {wr.status === 'pending' && showRejectForm && (
          <div className="space-y-3">
            <textarea
              ...
              rows={3}
              className="w-full p-4 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-xs font-quicksand font-bold outline-none focus:border-red-500 transition-all resize-none"
            />
            <div className="flex gap-3">
              <button onClick={() => onReject(wr._id, rejectReason)} disabled={!!processing || rejectReason.trim().length < 5}
                className="flex-1 h-12 bg-red-500 text-white rounded-2xl font-quicksand font-bold text-xs disabled:opacity-50 flex items-center justify-center gap-2">
                {processing === 'reject' ? <Loader2 className="size-4 animate-spin" /> : <XCircle className="size-4" />} Confirm Rejection
              </button>
              <button onClick={() => setShowRejectForm(false)}
                className="h-12 px-5 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-xs font-quicksand font-bold">
                Cancel
              </button>
            </div>
          </div>
        ...
        {(wr.status === 'approved' || wr.status === 'processing_error') && (
          <button onClick={() => onRecheck(wr._id)} disabled={!!processing}
            className="w-full h-12 bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 rounded-2xl font-quicksand font-bold text-xs flex items-center justify-center gap-2 hover:bg-indigo-500/20 disabled:opacity-50 transition-all">
      ...
      <div className="px-4 md:px-8 py-8 w-full space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-quicksand font-bold tracking-tight">Payout Queue</h1>
              <p className="text-xs text-[var(--text-secondary)] opacity-60 font-quicksand font-bold tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            <div className="flex flex-wrap gap-1 p-1 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl">
              {TABS.map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-xl text-[10px] lg:text-[12px] font-quicksand font-bold tracking-tight transition-all capitalize ${filter === f ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text-secondary)] opacity-50 hover:opacity-100'}`}>
                  {f}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-2 bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-2xl px-3 py-2">
               ...
               <select 
                 ...
                 className="bg-transparent text-[10px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] outline-none"
               >
            ...
            <div className="relative flex-1 w-full sm:w-auto">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full h-11 pl-10 pr-5 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-2xl text-xs font-quicksand font-bold outline-none focus:border-[var(--accent)] transition-all" />
            </div>
          </div>
          ...
          <div className="space-y-2 min-h-[300px]">
            {loading ? (
              ...
              <div className="py-20 text-center border border-dashed border-[var(--glass-border)] rounded-[2rem] opacity-30">
                ...
                <p className="text-sm font-quicksand font-bold">No withdrawal requests in this filter</p>
              </div>
            ) : displayed.map(w => {
              ...
              return (
                <div key={w._id} onClick={() => setSelected(w)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-quicksand font-bold text-sm text-[var(--text-primary)] truncate">{w.requestedBy?.name || '—'}</p>
                      <span className="text-[10px] lg:text-[12px] font-quicksand font-bold tracking-widest px-1.5 py-0.5 rounded-full bg-[var(--bg-secondary)] text-[var(--text-secondary)] opacity-60 shrink-0">
                        {w.role?.toUpperCase()}
                      </span>
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-base font-quicksand font-bold tabular-nums">{fmt(w.amount)}</p>
                    <p className="text-[10px] lg:text-[12px] font-quicksand font-bold opacity-30">{w.currency}</p>
                  </div>
