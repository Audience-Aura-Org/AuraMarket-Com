    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <header className="h-16 lg:h-20 flex items-center justify-between px-4 lg:px-6 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] backdrop-blur-xl shrink-0 z-10 text-[var(--text-primary)]">
        <div className="flex items-center gap-4 flex-1">
          ...
          <h2 className="text-lg lg:text-xl font-quicksand font-bold text-[var(--text-primary)] tracking-tight ">Global <span className="text-[var(--accent)]">Assets</span></h2>
          ...
          <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-[var(--bg-secondary)] rounded-xl border border-[var(--glass-border)]">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight w-64 placeholder:opacity-40"
              value={search}
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          <div className="flex bg-[var(--bg-secondary)] p-1 rounded-xl border border-[var(--glass-border)]">
            {['all', 'active', 'pending', 'archived'].map((s) => {
              const baseBtn = 'px-3 lg:px-4 py-1.5 lg:py-2 rounded-lg text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight transition-all ';
        ...
        <div className="p-4 lg:p-10 space-y-6 lg:space-y-10 pb-32">
          ...
          <div className="md:hidden flex items-center gap-2 px-4 py-3 bg-[var(--bg-secondary)] rounded-xl border border-[var(--glass-border)]">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight w-full placeholder:opacity-40"
              value={search}
              onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }}
            />
          </div>
          ...
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center opacity-40">
              ...
              <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold  tracking-[0.5em]">Extracting asset data...</p>
            </div>
          ) : (
            <div className="space-y-12">
                    ...
                    <div key={p._id} className={`p-6 lg:p-8 glass-panel border rounded-[32px] lg:rounded-[48px] group transition-all flex flex-col h-fit relative overflow-hidden shadow-sm hover:shadow-xl shrink-0 ${isSelected ? 'bg-[var(--accent)]/5 border-[var(--accent)]' : 'border-[var(--glass-border)] bg-[var(--bg-primary)]/50 hover:border-[var(--accent)]/40'}`}>
                      ...
                      <div className="aspect-[1.5] lg:aspect-[1.6] rounded-[24px] lg:rounded-[32px] overflow-hidden border border-[var(--glass-border)] mb-6 lg:mb-8 bg-[var(--bg-secondary)]/50 relative shadow-inner shrink-0 text-white">
                        ...
                        <div className="absolute top-3 right-3 lg:top-4 lg:right-4 flex gap-2">
                          <span className={'px-3 lg:px-4 py-1 lg:py-1.5 rounded-full text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight border backdrop-blur-md shadow-2xl ' + statusClass}>
                            {p.status}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-base lg:text-xl font-quicksand font-bold tracking-tight  group-hover:text-[var(--accent)] transition-colors truncate">{p.name}</h4>
                        ...
                        <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--accent)] mb-4 lg:mb-6 flex items-center gap-2 opacity-70">
                        ...
                        <div className="grid grid-cols-2 gap-3 lg:gap-4 mb-6 lg:mb-8">
                          <div className="p-3 lg:p-4 rounded-2xl bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] text-center shadow-inner">
                            <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight mb-1 opacity-40 ">Price Node</p>
                            <p className="text-xs lg:text-sm font-quicksand font-bold text-[var(--text-primary)]">{p.price?.toLocaleString()} <span className="text-[10px] lg:text-[12px] opacity-40">XAF</span></p>
                          </div>
                          <div className="p-3 lg:p-4 rounded-2xl bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] text-center shadow-inner">
                            <p className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight mb-1 opacity-40 ">Rating Node</p>
                            <div className="flex items-center justify-center gap-1">
                              <Star className="size-2.5 lg:size-3 text-amber-500 fill-amber-500" />
                              <span className="text-[10px] lg:text-[12px] lg:text-[11px] lg:text-[12px] font-quicksand font-bold">{p.rating || '0.0'}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3 lg:gap-4 pt-4 lg:pt-6 border-t border-[var(--glass-border)]/50">
                        {p.status !== 'active' ? (
                          <button
                            ...
                            className="flex-1 h-12 lg:h-14 rounded-xl lg:rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 font-quicksand font-bold text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] tracking-tight  hover:bg-emerald-500 hover:text-white transition-all shadow-xl shadow-emerald-500/5 flex items-center justify-center gap-2"
                          >
                          ...
                          <button
                            ...
                            className="flex-1 h-12 lg:h-14 rounded-xl lg:rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500 font-quicksand font-bold text-[10px] lg:text-[12px] lg:text-[10px] lg:text-[12px] tracking-tight  hover:bg-rose-500 hover:text-white transition-all shadow-xl shadow-rose-500/5 flex items-center justify-center gap-2"
                          >
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full py-32 lg:py-40 text-center opacity-30">
                  ...
                  <p className="text-[10px] lg:text-[12px] lg:text-xs font-quicksand font-bold  tracking-[0.5em]">No synchronization nodes found</p>
                </div>
              )}
            </div>
          )}
        ...
        {selectedIds.length > 0 && (
          ...
          >
            <div className="flex flex-col">
              <span className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--accent)]">Batch Selection</span>
              <span className="text-sm font-quicksand font-bold">{selectedIds.length} Asset Nodes</span>
            </div>

            <div className="flex items-center gap-4">
              <button 
                onClick={() => setSelectedIds([])}
                className="px-4 py-2 rounded-xl text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight hover:bg-[var(--bg-secondary)] transition-all"
              >
                Cancel
              </button>
              <button 
                ...
                className="px-8 py-4 rounded-full bg-rose-500 text-white text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight hover:bg-rose-600 transition-all flex items-center gap-2 shadow-lg shadow-rose-500/30 disabled:opacity-50"
              >
