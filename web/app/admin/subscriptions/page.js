      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10 w-full">
        <header className="h-20 flex items-center justify-between px-10 glass-panel border-b border-[var(--glass-border)] relative z-10 bg-[var(--bg-primary)] text-[var(--text-primary)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-quicksand font-bold tracking-tight text-[var(--text-primary)]">Subscription Management</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <p className="text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-emerald-600 ">System Live</p>
              </div>
            </div>
          </div>
          <div className="hidden md:flex items-center bg-[var(--bg-secondary)] rounded-full px-4 py-2 border border-[var(--glass-border)] w-80">
            ...
            <input className="bg-transparent border-none focus:ring-0 text-sm w-full placeholder:text-[var(--text-secondary)]/50 font-quicksand font-bold text-[var(--text-primary)]" placeholder="Search vendors, plans, or invoices..." />
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto w-full">
          <div className="p-10 space-y-8 w-full">
            <div>
              <h2 className="text-3xl font-quicksand font-bold text-[var(--text-primary)] tracking-tight">Subscription Management</h2>
              <p className="text-[var(--text-secondary)] mt-1 font-quicksand font-bold">Real-time overview of your vendor ecosystem and recurring revenue.</p>
            </div>
            ...
            <div className="bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] rounded-3xl overflow-hidden glass-panel shadow-sm">
              <div className="p-6 border-b border-[var(--glass-border)] flex flex-wrap items-center justify-between gap-4 bg-[var(--bg-primary)]/50">
                <div className="flex items-center gap-3">
                  <h2 className="text-xl font-quicksand font-bold text-[var(--text-primary)]">Vendor Subscription Overview</h2>
                  <span className="bg-[var(--accent)]/10 text-[var(--accent)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight px-2.5 py-0.5 rounded-full border border-[var(--accent)]/20 ">Live Data</span>
                </div>
                <div className="flex items-center gap-3">
                  <button className="flex items-center gap-2 px-4 py-2 bg-[var(--bg-secondary)]/50 rounded-xl border border-[var(--glass-border)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all ">
                    <Filter className="w-4 h-4" /> Filter
                  </button>
                  <button className="flex items-center gap-2 px-4 py-2 bg-[var(--accent)] rounded-xl text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight shadow-lg shadow-[var(--accent)]/20 hover:opacity-90 transition-all text-white ">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-[var(--bg-secondary)]/30 text-[var(--text-secondary)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight border-b border-[var(--glass-border)] ">
                    ...
                    {loading ? [...Array(5)].map((_, i) => (
                      ...
                      <tr key={i} className="group hover:bg-[var(--accent)]/5 transition-colors">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-[var(--bg-primary)] border border-[var(--glass-border)] flex items-center justify-center font-quicksand font-bold text-[var(--accent)] text-lg shadow-sm">
                              {s.vendor[0]}
                            </div>
                            <div>
                              <p className="text-sm font-quicksand font-bold text-[var(--text-primary)]">{s.vendor}</p>
                              <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] font-quicksand font-bold tracking-tight ">{s.id}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <span className={`inline-flex items-center px-3 py-0.5 rounded-full text-xs font-quicksand font-bold  tracking-tight border ${PLAN_BADGE[s.plan]}`}>
                            {s.plan}
                          </span>
                        </td>
                        <td className="px-6 py-5">
                          <p className="text-sm font-quicksand font-bold text-[var(--text-primary)]">{s.renewal}</p>
                          <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] font-quicksand font-bold tracking-tight mt-0.5 ">{s.billing}</p>
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-2">
                            <span className={`w-2.5 h-2.5 rounded-full ${PAYMENT_DOT[s.payment]} ${s.payment === 'overdue' ? 'animate-pulse' : ''} shadow-sm`} />
                            <span className="text-sm font-quicksand font-bold capitalize text-[var(--text-primary)]">{s.payment}</span>
                          </div>
                        </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20">
              <div className="bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] rounded-3xl p-8 glass-panel">
                <h3 className="text-lg font-quicksand font-bold tracking-tight text-[var(--text-primary)] mb-6 ">Plan Tier Distribution</h3>
              ...
              <div className="bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] rounded-3xl p-8 flex items-center gap-6 glass-panel">
                <div className="flex-1">
                   <h3 className="text-lg font-quicksand font-bold tracking-tight text-[var(--text-primary)] mb-2 ">Churn Prediction</h3>
                   <p className="text-sm text-[var(--text-secondary)] mb-6 font-quicksand font-bold">Based on payment delays and account activity in the last 30 days.</p>
                   <div className="flex items-end gap-2">
                      <span className="text-4xl font-quicksand font-bold text-[var(--text-primary)] tracking-tighter">2.4%</span>
                      <span className="text-emerald-600 text-[11px] lg:text-[12px] font-quicksand font-bold mb-2 flex items-center gap-1 tracking-tight">
                         <TrendingDown className="w-3 h-3" /> 0.8%
                      </span>
                   </div>
                   <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] tracking-[0.2em] font-quicksand font-bold mt-1  opacity-60">Stable Outlook</p>
                </div>
  ...
  return (
    <div className="bg-[var(--bg-primary)]/40 border border-[var(--glass-border)] rounded-3xl p-6 relative overflow-hidden group hover:translate-y-[-4px] transition-all glass-panel">
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        ...
        <span className={`text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight flex items-center gap-1  ${warn ? 'text-amber-600' : positive ? 'text-emerald-600' : 'text-red-600'}`}>
          {!warn && (positive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />)}
          {trend}
        </span>
      </div>
      <p className="text-[var(--text-secondary)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight  relative z-10">{label}</p>
      <h3 className="text-3xl font-quicksand font-bold text-[var(--text-primary)] mt-1 relative z-10">{value}</h3>
      {sub && <p className="text-[10px] lg:text-[12px] text-[var(--text-secondary)] mt-2 font-quicksand font-bold tracking-tight  opacity-60">{sub}</p>}
    </div>
  );
}
...
function TierBar({ label, pct, color }) {
  ...
  return (
    <div>
      <div className="flex justify-between text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight mb-2 ">
