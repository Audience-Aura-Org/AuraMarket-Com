  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col items-center justify-center px-4 py-12 text-[var(--text-primary)]">
      ...
      <div className="absolute top-6 left-6">
        <Link href={type === 'checkout' ? '/orders' : '/wallet'} className="flex items-center gap-2 text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="text-center space-y-3 mb-8">
              <h1 className={`text-2xl font-quicksand font-bold tracking-tighter ${cfg.color}`}>{cfg.title}</h1>
              <p className="text-[12px] font-quicksand font-bold text-[var(--text-secondary)] tracking-tight leading-relaxed opacity-70">
                {message}
              </p>
              {reason && (
                <div className="mt-4 p-4 rounded-2xl bg-red-500/10 border border-red-500/20">
                  <p className="text-[11px] lg:text-[12px] font-quicksand font-bold text-red-400 tracking-tight">{reason}</p>
                </div>
              )}
            </div>
            ...
            {state === 'successful' && balanceAdded > 0 && (
              <div className="mb-6 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-3">
                ...
                <p className="text-[12px] font-quicksand font-bold text-emerald-500 tracking-tight">
            ...
            <div className="space-y-3">
              {state === 'successful' && (
                <>
                  <button
                    ...
                    className="w-full h-12 rounded-2xl bg-emerald-500 text-white font-quicksand font-bold text-[11px] lg:text-[12px] tracking-tight flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-emerald-500/20"
                  >
                  ...
                  <button
                    ...
                    className="w-full h-12 rounded-2xl border border-[var(--glass-border)] text-[var(--text-secondary)] font-quicksand font-bold text-[11px] lg:text-[12px] tracking-tight hover:border-[var(--accent)]/40 hover:text-[var(--text-primary)] transition-all"
                  >
                    Continue Shopping
                  </button>
                </>
              ...
              {state === 'timeout' && (
                <>
                  <button
                    ...
                    className="w-full h-12 rounded-2xl bg-amber-500 text-white font-quicksand font-bold text-[11px] lg:text-[12px] tracking-tight flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-amber-500/20 disabled:opacity-50"
                  >
                    {recheckLoading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}
                    Recheck Payment Status
                  </button>
                  <Link href="/wallet/transactions" className="block w-full h-12 rounded-2xl border border-[var(--glass-border)] text-[var(--text-secondary)] font-quicksand font-bold text-[11px] lg:text-[12px] tracking-tight flex items-center justify-center gap-2 hover:border-[var(--accent)]/40 transition-all">
              ...
              {state === 'failed' && (
                <>
                  ...
                  <button
                    onClick={handleRecheck}
                    disabled={recheckLoading}
                    className="w-full h-12 rounded-2xl border border-[var(--glass-border)] text-[var(--text-secondary)] font-quicksand font-bold text-[11px] lg:text-[12px] tracking-tight flex items-center justify-center gap-2 hover:border-[var(--accent)]/40 transition-all disabled:opacity-50"
                  >
                    {recheckLoading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}
                    Recheck Payment
                  </button>
              ...
              {state === 'recheck' && (
                <div className="flex items-center justify-center gap-2 text-[11px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-60">
            ...
            {/* Reference display */}
            {ref && state !== 'loading' && (
              <p className="text-center text-[10px] lg:text-[12px] font-quicksand font-bold text-[var(--text-secondary)] opacity-30 tracking-tight mt-6">
                Ref: {ref}
              </p>
            )}
          </motion.div>
