  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[var(--bg-secondary)] gap-4">
      ...
      <button onClick={() => router.push('/shop')}
        className="px-6 py-2 text-xs font-bold tracking-normal text-white bg-[var(--accent)] rounded-full hover:opacity-90 transition-all">
        Back to Shop
      </button>
    </div>
  );
          ...
          <div className="order-1 md:order-2 flex-1 flex flex-col gap-4">
            ...
            >
              ...
              <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5">
                {!inStock && (
                  <span className="px-2.5 py-1 bg-red-500 text-white text-[10px] font-bold tracking-normal rounded-lg">
                    Out of Stock
                  </span>
                )}
                {discount && (
