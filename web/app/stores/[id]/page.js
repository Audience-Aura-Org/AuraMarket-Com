  ...
  if (!store) {
    return (
      <div className="min-h-screen bg-[var(--bg-secondary)] flex flex-col items-center justify-center text-center px-6">
        ...
        <h1 className="text-3xl font-quicksand font-bold text-[var(--text-primary)] mb-2  tracking-tighter">Store not found</h1>
        ...
        <button onClick={() => window.history.back()} className="mt-8 px-8 py-3 bg-[var(--text-primary)] text-[var(--bg-primary)] rounded-xl font-quicksand font-bold text-sm hover:opacity-90 transition-all">Go Back</button>
      </div>
    );
  }
              ...
              <div className="flex flex-row md:flex-col items-center gap-2 w-full md:w-auto shrink-0">
                ...
                {user?._id === store.vendor_id?.user_id?._id && (
                   <button 
                    ...
                    className="h-10 md:h-11 px-6 rounded-xl bg-[var(--accent)] text-white font-quicksand font-bold text-[10px] lg:text-[12px] tracking-tight  hover:brightness-110 transition-all shadow-lg shadow-[var(--accent)]/20 flex-1 md:flex-none flex items-center justify-center gap-2"
                   >
        ...
        <div className="px-6 lg:px-12">
            ...
            <div className="flex items-center gap-4 overflow-x-auto pb-4 w-full md:w-auto no-scrollbar">
               {[
                 ...
                 <motion.button 
                  ...
                  whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className={`h-11 md:h-12 px-6 md:px-8 rounded-2xl flex items-center gap-3 text-[10px] lg:text-[12px] md:text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight  transition-all shrink-0 border-2 ${
                    tab.id === activeTab
          ...
          {productsLoading ? (
              ...
              {(totalPages > 1 || products.length === 20 || page > 1) && (
                <div className="flex items-center justify-center gap-3 pb-32">
                   <button 
                    ...
                    className="px-6 py-3 rounded-xl bg-[var(--bg-primary)] border border-[var(--glass-border)] text-[11px] lg:text-[12px] font-quicksand font-bold tracking-tight  disabled:opacity-30 hover:bg-[var(--accent)] hover:text-white transition-all shadow-sm"
                   >
                     Previous
                   </button>
                      ...
                      {Array.from({ length: Math.max(totalPages, page + (products.length === 20 ? 1 : 0)) }, (_, i) => i + 1).map((p) => {
                         ...
                         return (
                            <button 
                               ...
                               className={`size-10 rounded-xl flex items-center justify-center text-[11px] lg:text-[12px] font-quicksand font-bold transition-all ${page === p ? 'bg-[var(--accent)] text-white shadow-lg shadow-[var(--accent)]/30' : 'bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)]'}`}
                            >
                               {p}
                            </button>
                         );
            ...
            <div className="bg-[var(--bg-primary)]/40 rounded-[3rem] md:rounded-[4rem] p-12 md:p-24 text-center border border-[var(--glass-border)] mb-32 glass-panel">
              ...
              <h3 className="text-2xl md:text-3xl font-quicksand font-bold text-[var(--text-primary)] mb-2  tracking-tighter">Inventory Dry</h3>
