  ...
  const renderAddressBox = (prefix, label, color, isMyBox) => {
    ...
    return (
      <div className={`rounded-2xl border p-4 space-y-3 ${isMyBox && user ? 'border-[var(--accent)]/20 bg-[var(--accent)]/[0.03]' : 'border-[var(--glass-border)] bg-[var(--bg-secondary)]'}`}>
        ...
        {/* My side: compact summary (auto-filled from profile) */}
        {isMyBox && user ? (
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-[var(--text-secondary)]">
              {[form[`${prefix}_street`], form[`${prefix}_quartier`], form[`${prefix}_district`], form[`${prefix}_city`]].filter(Boolean).join(', ') || 'No address set — update in Profile'}
            </p>
          </div>
        ) : (
