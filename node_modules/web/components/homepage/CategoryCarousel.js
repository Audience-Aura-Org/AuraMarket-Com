          ...
          >
            <div className="relative aspect-square rounded-[2rem] overflow-hidden bg-[var(--bg-primary)] border border-[var(--glass-border)] shadow-sm group-hover:shadow-xl group-hover:border-[var(--accent)]/30 transition-all duration-500">
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 to-transparent">
                <span className="text-white font-bold text-center block text-sm">
                  {cat.category_name}
                </span>
              </div>
            </div>
            <p className="mt-4 text-center font-bold text-[var(--text-primary)] opacity-80 group-hover:opacity-100 transition-opacity">
              {cat.category_name}
            </p>
          </Link>
        ))}
