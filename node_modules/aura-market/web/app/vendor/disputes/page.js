    ...
    <>
      <header className="min-h-20 lg:h-24 h-auto flex flex-col lg:flex-row lg:items-center justify-between px-6 lg:px-10 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] shrink-0 z-10 py-4 lg:py-0 gap-4 lg:gap-0 text-[var(--text-primary)]">
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          <p className="text-[var(--text-secondary)] text-[9px] font-black uppercase tracking-[0.3em] opacity-40"><span>{disputes.length}</span> Active Cases</p>
        </div>
      ...
      <div className="p-4 lg:p-10 space-y-6 lg:space-y-10 pb-32">
           ...
           <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-6 mt-2">
              <div className="glass-panel p-4 lg:p-6 rounded-[24px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 flex items-center gap-3 lg:gap-6 shadow-sm">
                 ...
                 <div className="min-w-0">
                    <p className="text-[7px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)] opacity-60">Pending Cases</p>
              ...
              <div className="glass-panel p-4 lg:p-6 rounded-[24px] lg:rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 flex items-center gap-3 lg:gap-6 shadow-sm">
                 ...
                 <div className="min-w-0">
                    <p className="text-[7px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)] opacity-60">Settled</p>
              ...
              <div className="hidden lg:flex glass-panel p-6 rounded-[40px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 items-center gap-6 shadow-sm col-span-2 lg:col-span-1">
                 ...
                 <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)] opacity-60">Oversight</p>
                    <h2 className="text-xl font-black tracking-tighter mt-1 uppercase text-indigo-500 px-3 py-1 bg-indigo-500/10 rounded-lg inline-block text-[9px] whitespace-nowrap">Admin Support Active</h2>
                 </div>
              </div>
           </div>
           ...
           <div className="relative w-full lg:w-96 group">
             ...
             <input 
               type="text" 
               placeholder="Search cases by ID or reason..." 
               value={search}
           ...
           {loading ? (
             ...
             <div className="py-20 lg:py-40 flex flex-col items-center justify-center text-center max-w-md mx-auto">
                ...
                <h3 className="text-xl lg:text-2xl font-black uppercase tracking-tighter text-[var(--text-primary)]">No Active Disputes</h3>
                <p className="text-[10px] lg:text-xs font-black uppercase tracking-[0.25em] text-[var(--text-secondary)] mt-4 opacity-40 leading-relaxed">Your store has no active appeals or case overrides at this time.</p>
             </div>
           ) : (
                  ...
                  <div key={d._id} className="glass-panel p-6 lg:p-10 rounded-[32px] lg:rounded-[56px] border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 hover:bg-[var(--bg-primary)]/60 transition-all shadow-2xl relative overflow-hidden flex flex-col lg:flex-row gap-6 lg:gap-10 group/row">
                     ...
                     <div className="lg:w-[320px] lg:border-r border-[var(--glass-border)] lg:pr-10 flex flex-col justify-between gap-6">
                        <div>
                           <div className="flex items-center gap-3">
                              <span className={`px-4 py-2 rounded-xl text-[8px] lg:text-[9px] font-black uppercase tracking-widest border transition-all ${
                                ...
                                {d.status === 'resolved' ? 'Case Settled' : 'Resolution Active'}
                              </span>
                           </div>
                           ...
                           <p className="text-[8px] lg:text-[10px] font-black tracking-widest uppercase text-[var(--text-secondary)] opacity-30 mt-1 lg:mt-2">CASE ID: {d._id.slice(-12).toUpperCase()}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4 p-3 lg:p-4 rounded-2xl bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)]">
                           ...
                           <div className="min-w-0">
                              <p className="text-[8px] lg:text-[9px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)] opacity-40">Buying Party</p>
                              <p className="text-xs lg:text-sm font-black truncate text-[var(--text-primary)]">{d.initiator_id?.name || 'Customer'}</p>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="flex-1 space-y-4 lg:space-y-6 flex flex-col justify-center">
                        <div className="p-5 lg:p-8 rounded-[24px] lg:rounded-[36px] bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)] shadow-inner relative group/note">
                           ...
                           <p className="text-[8px] lg:text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] opacity-30 mb-3 lg:mb-4">Case Details</p>
                        ...
                        {d.status === 'resolved' && d.admin_notes && (
                          <div className="p-5 lg:p-6 rounded-[24px] lg:rounded-[32px] bg-indigo-500/5 border border-indigo-500/20 shadow-lg shadow-indigo-500/5">
                             <p className="text-[8px] lg:text-[10px] font-black uppercase tracking-widest text-indigo-500 mb-2 flex items-center gap-2">
                                ...
                                Resolution Verdict ({d.resolution_type?.replace(/_/g, ' ')})
                             </p>
                     ...
                     <div className="lg:w-[280px] flex flex-col justify-between gap-6 lg:items-end lg:text-right pt-6 lg:pt-0 lg:border-l border-[var(--glass-border)] lg:pl-10">
                        <div className="space-y-2 lg:space-y-3">
                           <p className="text-[8px] lg:text-[10px] font-black uppercase tracking-[0.25em] text-[var(--text-secondary)] opacity-30">Order Reference</p>
