...
**Base URL:** `http://13.63.100.47:5000` (AWS Backend)
**Authentication:** Required (JWT Bearer Token)

**cURL Example:**
```bash
curl -X POST http://13.63.100.47:5000/api/upload/single \
...
**cURL Example:**
```bash
curl -X POST http://13.63.100.47:5000/api/upload/multiple \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -F "images=@image1.jpg" \
  -F "images=@image2.jpg" \
  -F "images=@image3.jpg" \
...
async function uploadImage(file, token, type = 'products') {
  ...
  const response = await fetch('http://13.63.100.47:5000/api/upload/single', {
    method: 'POST',
    headers: {
...
async function uploadImages(files, token, type = 'products') {
  ...
  const response = await fetch('http://13.63.100.47:5000/api/upload/multiple', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`
    },
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await fetch(
        'http://13.63.100.47:5000/api/upload/single',
        {
          method: 'POST',
          headers: {
...
```
POST http://13.63.100.47:5000/api/upload/single
...
```
POST http://13.63.100.47:5000/api/upload/multiple
