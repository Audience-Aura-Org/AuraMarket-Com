...
export default function DashboardLayout({ children, role, hideSidebar = false, hideFooter = false }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--bg-secondary)] text-[var(--text-primary)] transition-colors duration-500 relative overflow-x-hidden">
      ...
      <div className={`flex-1 flex flex-col relative z-[210] w-full ${!hideSidebar ? 'lg:pl-[240px]' : ''}`}>
        ...
        {!hideSidebar && !hideFooter && <div className="relative z-20 w-full"><Footer /></div>}
      </div>
    </div>
  );
}
