...
export default function AdminOrdersPage() {
  ...
  return (
    <>
      <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-[var(--nav-border)] bg-[var(--nav-bg)] shrink-0 z-10 text-[var(--nav-text)]">
        <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
          {[
            ...
            <div key={s.label} className="p-4 lg:p-5 rounded-2xl lg:rounded-3xl glass-panel border border-[var(--glass-border)] bg-[var(--bg-primary)]/40 hover:border-[var(--accent)]/20 transition-all shadow-sm">
              <p className="text-[7px] lg:text-[8px] font-black tracking-widest text-[var(--text-secondary)] uppercase opacity-50">{s.label}</p>
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex-shrink-0 w-24 lg:w-32">
                          <p className="font-black text-[var(--text-primary)] text-[10px] lg:text-[12px] tracking-widest font-mono uppercase">#{order._id?.slice(-8)}</p>
                          <p className="text-[8px] lg:text-[9px] text-[var(--text-secondary)] font-black mt-0.5 opacity-50 uppercase tracking-widest">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-black text-[var(--text-primary)] text-xs lg:text-sm truncate uppercase tracking-tight">{customer?.name || 'Unknown'}</p>
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-6 lg:gap-12 border-t sm:border-t-0 pt-4 sm:pt-0 border-[var(--glass-border)]/20">
                        <div className="hidden lg:block w-40">
                          <span className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em] opacity-40 block mb-1">Store Node</span>
                          {order.vendor_id && (
                            <Link 
                        ...
                        <div className="text-right flex-shrink-0">
                          <span className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-[0.2em] opacity-40 block mb-1 sm:hidden">Total Amount</span>
                        ...
                        <div className="flex items-center gap-2 lg:gap-3 shrink-0">
                           <div className={`px-2.5 py-1 rounded-lg text-[7px] lg:text-[8px] font-black tracking-widest border uppercase transition-all ${status.bg} ${status.color} ${status.border} shadow-sm`}>
                             {status.label}
                           </div>
                      ...
                      <div className="border-t border-[var(--glass-border)]/50 divide-y sm:divide-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 animate-in fade-in slide-in-from-top-4 duration-300">
                        <div className="p-6 lg:p-8 space-y-6 bg-gradient-to-br from-transparent to-[var(--bg-secondary)]/30">
                           <div className="flex items-center gap-3">
                              ...
                              <h4 className="text-[10px] font-black tracking-widest text-[var(--text-primary)] uppercase">Customer Identity</h4>
                           </div>
                           <div className="space-y-4">
                              <div className="flex gap-4 p-4 rounded-2xl bg-[var(--bg-primary)]/50 border border-[var(--glass-border)] shadow-inner">
                                 ...
                                 <div className="min-w-0">
                                    <p className="font-black text-sm uppercase truncate">{customer?.name}</p>
                                    <p className="text-[9px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mt-0.5">{customer?.email}</p>
                                 </div>
                              </div>
                              <div className="grid grid-cols-2 gap-2">
                                 {[
                                   ...
                                   <div key={it.label} className="p-3 rounded-xl bg-[var(--bg-secondary)]/50 border border-[var(--glass-border)]">
                                      <p className="text-[7px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1 opacity-50">{it.label}</p>
                        ...
                        <div className="p-6 lg:p-8 lg:col-span-2 bg-gradient-to-bl from-transparent to-[var(--bg-secondary)]/50">
                           <div className="flex items-center justify-between mb-6">
                              <div className="flex items-center gap-3">
                                 ...
                                 <h4 className="text-[10px] font-black tracking-widest text-[var(--text-primary)] uppercase">Manifest Data</h4>
                              </div>
                              {order.vendor_id && (
                                <Link 
                                  href={`/stores/${order.vendor_id._id}`}
                                ...
                                >
                                  ...
                                  <span className="text-[9px] font-bold text-[var(--accent)] uppercase tracking-widest truncate max-w-[120px]">
                                    {order.vendor_id?.store_name}
                                  </span>
                                </Link>
                              )}
                           ...
                           <div className="space-y-4">
                              {products.map((item, i) => (
                                <div key={i} className="flex items-center gap-4 p-3 lg:p-4 rounded-2xl bg-[var(--bg-primary)]/80 border border-[var(--glass-border)] group hover:border-[var(--accent)]/30 transition-all shadow-sm">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="font-black text-[var(--text-primary)] text-xs lg:text-sm uppercase truncate mb-1">{item.name}</p>
                                    <div className="flex items-center gap-3">
                                      <span className="px-2 py-0.5 rounded bg-[var(--bg-secondary)] text-[8px] lg:text-[9px] font-black text-[var(--accent)] uppercase tracking-widest">x{item.quantity}</span>
                                      <span className="text-[9px] font-black text-[var(--text-secondary)] opacity-50 uppercase tracking-widest">{item.price?.toLocaleString()} XAF / UNIT</span>
                                    </div>
                                  </div>
                              ...
                              <div className="mt-6 p-5 lg:p-6 rounded-[24px] bg-[var(--accent)]/5 border border-[var(--accent)]/10 flex flex-col gap-3 shadow-inner">
                                 ...
                                 <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em]">Total Resolution</span>
