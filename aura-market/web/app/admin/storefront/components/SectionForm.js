    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-12">
      ...
      <div className="relative w-full max-w-5xl max-h-full bg-[var(--bg-primary)] rounded-[3rem] border border-[var(--glass-border)] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
                ...
                {productSearchResults.map(product => (
                  <div key={product._id} className="glass-panel p-4 rounded-2xl border border-[var(--glass-border)] flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                           <span className="text-[9px] font-black uppercase tracking-widest bg-[var(--accent)]/10 text-[var(--accent)] px-2 py-0.5 rounded border border-[var(--accent)]/20">{product.brand}</span>
                    ...
                    <button 
                      ...
                      className="bg-white/5 border border-white/10 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[var(--accent)] hover:text-white transition-all flex items-center gap-2"
                    >
        ...
        <div className="p-8 border-b border-[var(--glass-border)] flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-[var(--accent)]/10 text-[var(--accent)] px-4 py-2.5 rounded-xl font-bold flex items-center gap-2 text-[10px] uppercase tracking-widest border border-[var(--accent)]/30 hover:bg-[var(--accent)]/20 transition-all"
            >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-12 no-scrollbar">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest opacity-40 ml-1">Section Type</label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest opacity-40 ml-1">Display Title</label>
              <input 
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button 
                type="button"
                onClick={addDataItem}
                className="text-xs font-black uppercase tracking-widest text-[var(--accent)] flex items-center gap-2 hover:opacity-80"
              >
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {(['hero', 'promo_banner', 'categories'].includes(formData.type)) && (
                      <div className="space-y-2 md:col-span-2">
                         <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Image URL / Upload</label>
                    ...
                    {(['hero', 'promo_banner'].includes(formData.type)) && (
                      <>
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Subtext</label>
                          <input 
                            value={item.subtext}
                        ...
                        <div className="space-y-2 md:col-span-2">
                          <label className="text-[10px] font-black uppercase tracking-widest opacity-40">CTA Button Text (Optional)</label>
                          <input 
                            value={item.cta_text}
                    ...
                    {(['featured_products', 'collection', 'trending'].includes(formData.type)) && (
                      <div className="space-y-2 md:col-span-2">
                         <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Search & Select Product</label>
                                     ...
                                     {itemProductResults.map(p => (
                                       ...
                                       >
                                         ...
                                         <div className="min-w-0">
                                            ...
                                            <p className="text-[10px] text-white/40 font-mono truncate">{p._id}</p>
                                         </div>
                                       </div>
                                     ))}
                                  </div>
                         ...
                         {item.product_id && typeof item.product_id !== 'object' && (
                           <div className="px-4 py-1.5 bg-[var(--accent)]/5 rounded-lg border border-[var(--accent)]/10 inline-flex items-center gap-2">
                             ...
                             <span className="text-[9px] font-black uppercase text-[var(--accent)]">ID: {item.product_id}</span>
                           </div>
                         )}
                      </div>
                    )}
                    ...
                    {(['categories'].includes(formData.type)) && (
                      <div className="space-y-2">
                         <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Category Selection</label>
                               ...
                               <div className="absolute top-full left-0 right-0 mt-2 bg-[#120a12] border border-white/20 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-[100] p-2 space-y-2 animate-in fade-in slide-in-from-top-2">
                                  ...
                                  <div className="max-h-48 overflow-y-auto no-scrollbar">
                                     ...
                                     {filteredCategories.length === 0 && (
                                       <div className="text-[10px] text-center py-4 opacity-40 uppercase tracking-widest">No matches</div>
                                     )}
                                  </div>
                               </div>
                            )}
                     ...
                     <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Link Destination</label>
                               ...
                               <div className="absolute top-full left-0 right-0 mt-2 bg-[#120a12] border border-white/20 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-[100] p-2 space-y-2 animate-in fade-in slide-in-from-top-2">
                                  ...
                                  <div className="max-h-48 overflow-y-auto no-scrollbar">
                                     <div 
                                       ...
                                       className="flex items-center px-4 py-3 rounded-xl cursor-pointer hover:bg-white/10 text-xs text-[var(--accent)] font-black uppercase tracking-widest transition-all"
                                     >
                                        [ Generic Shop Page ]
                                     </div>
                                     {filteredCategories.map(cat => (
                    ...
                    {(formData.type === 'stores') && (
                      <div className="space-y-2 md:col-span-2">
                         <label className="text-[10px] font-black uppercase tracking-widest opacity-40">Search & Select Vendor</label>
                                     ...
                                     {itemVendorResults.map(v => (
                                       ...
                                       >
                                         ...
                                         <div className="min-w-0">
                                            ...
                                            <p className="text-[10px] text-white/40 font-mono truncate">{v._id}</p>
                                         </div>
                                       </div>
                                     ))}
                                  </div>
                         ...
                         {item.vendor_id && typeof item.vendor_id !== 'object' && (
                           <div className="px-4 py-1.5 bg-[var(--accent)]/5 rounded-lg border border-[var(--accent)]/10 inline-flex items-center gap-2">
                             ...
                             <span className="text-[9px] font-black uppercase text-[var(--accent)]">ID: {item.vendor_id}</span>
                           </div>
                         )}
                      </div>
                    )}
          ...
          <div className="grid md:grid-cols-2 gap-12 pt-8 border-t border-[var(--glass-border)] pb-10">
             <div className="space-y-6">
                <h4 className="font-black flex items-center gap-2 tracking-widest uppercase text-[10px] text-[var(--accent)]">
                ...
                <div className="flex items-center justify-between p-5 bg-white/5 rounded-3xl border border-white/5">
                   <span className="text-sm font-bold opacity-60 uppercase tracking-widest text-[10px]">Layout Mode</span>
                   <select 
                ...
                <div className="flex items-center justify-between p-5 bg-white/5 rounded-3xl border border-white/5">
                   <div className="space-y-0.5">
                      <span className="text-sm font-bold opacity-60 uppercase tracking-widest text-[10px]">Status</span>
                ...
                {formData.type === 'hero' && (
                  <div className="flex items-center justify-between p-5 bg-white/5 rounded-3xl border border-white/5">
                    <div className="space-y-0.5">
                       <span className="text-sm font-bold opacity-60 uppercase tracking-widest text-[10px]">Autoplay</span>
             ...
             <div className="space-y-6">
                <h4 className="font-black flex items-center gap-2 tracking-widest uppercase text-[10px] text-blue-400">
                   <Clock className="w-4 h-4" /> Scheduling
                </h4>
                <div className="grid gap-4">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase opacity-40 ml-1">Starts (Optional)</label>
                      <input 
                        type="datetime-local" 
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase opacity-40 ml-1">Ends (Optional)</label>
                      <input 
                        type="datetime-local" 
        ...
        <div className="p-8 border-t border-[var(--glass-border)] flex items-center justify-end gap-4 bg-[var(--bg-secondary)] relative z-30">
          <button onClick={onClose} className="px-8 py-4 rounded-2xl font-bold opacity-60 hover:opacity-100 transition-all text-xs uppercase tracking-widest">
            Cancel
          </button>
          <button 
            disabled={loading}
            onClick={handleSubmit} 
            className="bg-[var(--accent)] text-white px-12 py-4 rounded-2xl font-black shadow-2xl shadow-[var(--accent)]/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 text-xs uppercase tracking-[0.2em]"
          >
