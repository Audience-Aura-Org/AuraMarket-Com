      ...
      <div className="px-6 py-6 border-b border-[var(--glass-border)] bg-[var(--bg-secondary)]/10 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black uppercase tracking-tight">Merchant Registry</h1>
              <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 uppercase tracking-widest">Global Vendor Oversight</p>
            </div>
          </div>
            ...
            <div className="flex bg-[var(--bg-secondary)] border border-[var(--glass-border)] rounded-xl p-0.5">
              {['all', 'verified', 'unverified'].map(s => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${statusFilter === s ? 'bg-[var(--accent)] text-white' : 'text-[var(--text-secondary)] opacity-40'}`}
                >
                  {s}
                </button>
              ))}
           ...
           <div className="space-y-2 min-h-[600px]">
              {currentVendors.map(v => (
                <div key={v._id} className="flex items-center gap-4 p-3 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] hover:border-[var(--accent)]/30 transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <p className="text-[11px] font-black uppercase truncate">{v.store_name}</p>
                      {v.verified && <ShieldCheck className="size-3 text-emerald-500" />}
                    </div>
                    <p className="text-[9px] font-bold text-[var(--text-secondary)] opacity-40 truncate uppercase mt-0.5">Owner: {v.user_id?.name}</p>
                  </div>
                  ...
                  <div className="hidden lg:grid grid-cols-3 gap-8 px-8 border-x border-[var(--glass-border)]">
                     <div className="text-center">
                        ...
                        <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-20 uppercase tracking-widest">Volume</p>
                     </div>
                     <div className="text-center">
                        <p className="text-xs font-black">{fmt(v.total_revenue)}</p>
                        <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-20 uppercase tracking-widest">Revenue</p>
                     </div>
                     <div className="text-center">
                        ...
                        <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-20 uppercase tracking-widest">Rating</p>
                     </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                     <div className="text-right">
                        <div className="flex items-center gap-2 justify-end">
                           ...
                           <span className="text-[9px] font-black uppercase tracking-widest opacity-60">{v.verified ? 'Verified' : 'Pending'}</span>
                        </div>
                        <p className="text-[8px] font-black text-[var(--text-secondary)] opacity-20 uppercase mt-0.5 tracking-widest">Merchant State</p>
                     </div>
              ...
              {filteredVendors.length === 0 && (
                 <div className="py-20 text-center border border-dashed border-[var(--glass-border)] rounded-[2rem] opacity-20">
                    <p className="text-xs font-black uppercase tracking-widest">No merchant nodes synchronized</p>
                 </div>
              )}
           </div>
        )}
