      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10 w-full">
        ...
        <header className="h-16 flex items-center justify-between px-10 glass-panel border-b border-[var(--glass-border)] bg-[var(--bg-primary)]/50">
          <div className="flex items-center gap-6">
            <h2 className="text-xl font-black text-[var(--text-primary)] tracking-tight uppercase">Queue <span className="text-[var(--accent)]">Control</span></h2>
            <div className="h-4 w-px bg-[var(--glass-border)]" />
            <div className="flex items-center gap-2">
               <button 
                 onClick={() => setActiveTab('Vendors')} 
                 className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'Vendors' ? 'bg-[var(--accent)] text-white' : 'hover:bg-[var(--accent)]/10'}`}
               >
                 Vendors
               </button>
               <button 
                 onClick={() => setActiveTab('Products')} 
                 className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'Products' ? 'bg-[var(--accent)] text-white' : 'hover:bg-[var(--accent)]/10'}`}
               >
                 Products
               </button>
            </div>
          ...
          <div className="flex items-center gap-4">
             <button onClick={fetchData} className="p-2 rounded-lg border border-[var(--glass-border)] hover:bg-[var(--accent)]/10 transition-all">
                <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
             </button>
             <div className="px-4 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 text-[9px] font-black tracking-widest uppercase">
                System Active
             </div>
          </div>
        </header>

        <div className="flex-1 overflow-y-auto w-full p-10 no-scrollbar">
