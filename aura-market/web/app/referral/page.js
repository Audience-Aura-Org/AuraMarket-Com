      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <div className="sticky top-0 z-50 bg-[var(--bg-secondary)]/80 backdrop-blur-xl px-8 py-8 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <h1 className="text-4xl font-black tracking-tighter uppercase text-[var(--text-primary)]">Aura Rewards</h1>
          </div>
        ...
        <main className="px-8 space-y-12 mt-4">
          ...
          <section className="relative overflow-hidden p-12 rounded-[48px] bg-[var(--bg-primary)]/80 border border-[var(--accent)]/20 shadow-2xl glass-panel">
            <div className="relative z-10">
              <p className="text-[10px] font-black text-[var(--accent)] tracking-[0.4em] mb-4 uppercase opacity-80">Accumulated Points</p>
              ...
              <div className="mt-12 flex items-center gap-6">
                ...
                <span className="text-[10px] font-black tracking-widest text-[var(--text-secondary)] whitespace-nowrap uppercase">70% to Gold Tier</span>
              </div>
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-8 px-2">
              <h3 className="text-2xl font-black tracking-tight uppercase">Redeem Nexus</h3>
              <button className="text-[10px] font-black tracking-[0.2em] text-[var(--accent)] hover:underline transition-all uppercase">View Matrix</button>
            </div>
          ...
          <section className="pb-12">
            <h3 className="text-2xl font-black tracking-tight mb-8 px-2 uppercase">Sync Activity</h3>
            <div className="space-y-4">
              <ActivityRow 
  ...
  return (
    <div className={`min-w-[200px] bg-[var(--bg-primary)] border border-[var(--glass-border)] p-8 rounded-[40px] flex flex-col gap-6 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 ${disabled ? 'opacity-40 grayscale-[0.5]' : ''}`}>
      ...
      <div>
        ...
        <p className="text-[10px] font-black tracking-widest text-[var(--text-secondary)] mt-2 uppercase opacity-60">{points} AUR Points</p>
      </div>
      <button disabled={disabled} className={`w-full py-4 text-[10px] font-black tracking-widest rounded-2xl transition-all uppercase mt-2 shadow-sm ${color} ${!disabled && 'hover:scale-95'}`}>
        {action}
      </button>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-between p-8 bg-[var(--bg-primary)]/80 border border-[var(--glass-border)] rounded-[32px] hover:bg-[var(--bg-primary)] transition-all shadow-sm group">
      <div className="flex items-center gap-6">
        ...
        <div>
          <p className="text-xl font-black tracking-tight text-[var(--text-primary)] uppercase">{title}</p>
          <p className="text-[10px] font-black tracking-widest text-[var(--text-secondary)] mt-1.5 uppercase opacity-30">{subtitle}</p>
        </div>
      </div>
