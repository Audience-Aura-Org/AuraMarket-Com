...
import { 
  ...
  Lock, ArrowRightLeft, Sparkles, Building2, RefreshCw
} from 'lucide-react';
...
export default function WalletPage() {
  ...
  const fetchWallet = async () => {
    try {
      ...
      if (balRes.data.success) {
        ...
        setPendingBalance(balRes.data.data?.pending_escrow || balRes.data.pending_escrow || 0);
      }
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setBalance(res.data.data?.new_balance || res.data.new_balance);
        const newTx = res.data.data?.transaction || res.data.transaction;
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setBalance(res.data.data?.remaining_balance || res.data.remaining_balance);
        const newTx = res.data.data?.transaction || res.data.transaction;
        if (newTx) setTransactions(prev => [Array.isArray(newTx) ? newTx[0] : newTx, ...prev]);
  ...
  const totalIn = transactions.filter(t => ['deposit', 'refund', 'payout'].includes(t.type)).reduce((s, t) => s + (t.amount || 0), 0);
  const totalOut = transactions.filter(t => ['withdrawal', 'payment'].includes(t.type)).reduce((s, t) => s + (t.amount || 0), 0);
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="w-full max-w-md bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-3xl p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${modal === 'deposit' ? 'bg-emerald-500/10' : 'bg-[var(--accent)]/10'}`}>
                  {modal === 'deposit' ? (
                  ...
                  {modal === 'deposit' && (
                    <div className="flex gap-2 mt-3">
                      {[5000, 10000, 25000, 50000].map(qa => (
                        <button key={qa} onClick={() => setAmount(String(qa))} className="flex-1 py-2.5 rounded-xl text-[10px] font-black tracking-wider border border-[var(--glass-border)] bg-[var(--bg-secondary)] hover:border-[var(--accent)]/40 transition-all text-[var(--text-secondary)]">
        ...
        <div className="px-4 md:px-8 py-6 border-b border-[var(--glass-border)]">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={fetchWallet} className="p-2 rounded-xl border border-[var(--glass-border)] hover:bg-white/5 text-[var(--text-secondary)]">
        ...
        <div className="px-4 md:px-8 py-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">Available</span>
              </div>
              <p className="text-2xl font-black text-emerald-500">{balance.toLocaleString()}</p>
              <p className="text-[9px] text-emerald-500/60 mt-1">XAF</p>
            </div>
            
            <div className="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/20">
              <div className="flex items-center gap-3 mb-3">
                <Lock className="w-5 h-5 text-amber-500" />
                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">In Escrow</span>
              </div>
              <p className="text-2xl font-black text-amber-500">{pendingBalance.toLocaleString()}</p>
              <p className="text-[9px] text-amber-500/60 mt-1">XAF held</p>
            </div>
            
            <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
              <div className="flex items-center gap-3 mb-3">
                <ArrowDownLeft className="w-5 h-5 text-emerald-500" />
                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">Total In</span>
              </div>
              <p className="text-2xl font-black text-emerald-500">+{totalIn.toLocaleString()}</p>
              <p className="text-[9px] text-emerald-500/60 mt-1">XAF received</p>
            </div>
            
            <div className="p-5 rounded-2xl bg-red-500/5 border border-red-500/20">
              <div className="flex items-center gap-3 mb-3">
                <ArrowUpRight className="w-5 h-5 text-red-500" />
                <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider">Total Out</span>
              </div>
              <p className="text-2xl font-black text-red-500">-{totalOut.toLocaleString()}</p>
              <p className="text-[9px] text-red-500/60 mt-1">XAF sent</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <button
              ...
              className="flex items-center justify-center gap-3 p-5 rounded-2xl bg-[var(--bg-primary)] border border-[var(--glass-border)] text-[var(--text-primary)] font-bold text-sm uppercase tracking-wider hover:bg-[var(--bg-secondary)] transition-all active:scale-[0.98]"
            >
