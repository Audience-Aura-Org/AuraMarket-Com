...
import {
  TrendingUp, Package, DollarSign, BarChart3, ShoppingBag, Users, ArrowUpRight, Star, Activity, ArrowDownLeft, ArrowUpLeft
} from 'lucide-react';
  ...
  return (
    <DashboardLayout role="vendor">
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="hidden md:flex items-center justify-between px-4 lg:px-8 py-6 border-b border-[var(--glass-border)] bg-[var(--bg-primary)] backdrop-blur-xl shrink-0">
                      ...
                      {[...products]
                        ...
                        .map((p, i) => (
                          <div key={p._id} className="flex items-center gap-4 p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] hover:border-[var(--accent)]/20 transition-all">
                            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)] font-black text-sm">{i + 1}</div>
                            <div className="w-12 h-12 rounded-xl bg-[var(--bg-primary)] border border-[var(--glass-border)] overflow-hidden shrink-0">
                              <img src={p.images?.[0]?.url || `https://api.dicebear.com/7.x/shapes/svg?seed=${p._id}`} className="size-full object-cover" alt={p.name} />
                            </div>
