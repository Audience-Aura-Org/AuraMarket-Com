          ...
          <div className="lg:hidden flex items-center overflow-x-auto px-6 pb-4 no-scrollbar gap-2 -mt-2">
            {filteredTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`whitespace-nowrap px-6 py-3 md:py-3.5 rounded-full text-[10px] md:text-[11px] font-black tracking-widest uppercase transition-all ${
                  activeTab === tab.id
                ...
                <div className="space-y-12">
                  ...
                  <div className="bg-[var(--bg-primary)]/80 backdrop-blur-3xl border border-[var(--glass-border)] rounded-[3rem] p-8 md:p-12 relative overflow-hidden glass-panel group shadow-2xl mb-12 flex flex-col items-center justify-center text-center">
                    ...
                    <div className="space-y-4 relative z-10 w-full">
                       <h3 className="text-3xl md:text-5xl font-black tracking-tighter uppercase shadow-sm">
                       ...
                       <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                         <span className="px-4 py-1.5 rounded-full bg-[var(--accent)]/10 text-[var(--accent)] text-[9px] font-black tracking-widest uppercase border border-[var(--accent)]/20 shadow-sm">
                           {user?.role === 'vendor' ? 'Nexus Vendor' : user?.role === 'admin' ? 'Root Administrator' : user?.role === 'logistics' ? 'Logistics Node' : 'Node User'}
                         </span>
                         <span className="px-4 py-1.5 rounded-full bg-emerald-500/10 text-emerald-500 text-[9px] font-black tracking-widest uppercase border border-emerald-500/20 shadow-sm">
                           Active Session
                         </span>
                       </div>
                    </div>
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <div className="flex items-center gap-6 px-4 md:px-6">
        <h3 className="text-[10px] md:text-[11px] font-black tracking-[0.4em] uppercase text-[var(--accent)] shadow-sm">{title}</h3>
  ...
  return (
    <div className="space-y-3 px-1 md:px-2">
      <div className="flex items-center gap-3 ml-2 md:ml-4">
        ...
        <label className="text-[9px] md:text-[10px] font-black tracking-[0.2em] text-[var(--text-secondary)] uppercase">{label}</label>
      </div>
      {area ? (
        <textarea
          value={value}
...
function InputRow({ label, value, disable }) {
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between p-5 md:p-6 rounded-[2rem] bg-[var(--bg-secondary)]/30 border border-[var(--glass-border)] gap-2 md:gap-0">
      <span className="text-[10px] font-black tracking-[0.2em] uppercase text-[var(--text-secondary)]">{label}</span>
      <span className={`text-[11px] md:text-xs font-black uppercase tracking-wider ${disable ? 'opacity-60 text-[var(--text-secondary)]' : 'text-[var(--text-primary)]'}`}>{value}</span>
    </div>
  );
}
  ...
  return (
    <button className="relative w-full flex items-center justify-between p-5 md:p-6 rounded-[2rem] bg-[var(--bg-secondary)]/40 border border-[var(--glass-border)] hover:bg-[var(--accent)] hover:text-white group transition-all duration-300 text-left overflow-hidden hover:-translate-y-0.5 hover:shadow-xl">
      ...
      <div className="flex items-center gap-5 md:gap-6 relative z-10">
        ...
        <div>
          <p className="text-[11px] md:text-xs font-black tracking-[0.2em] uppercase transition-colors">{label}</p>
