      ...
      <div className="max-w-4xl mx-auto relative z-10 space-y-16">
           ...
           <div className="flex flex-col lg:flex-row items-end justify-between gap-8">
              <div className="space-y-4">
                 <div className="flex items-center gap-3 text-emerald-500 bg-emerald-500/10 w-fit px-4 py-1.5 rounded-full border border-emerald-500/20">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.4em]">All Systems Nominal</span>
                 </div>
                 <h1 className="text-6xl lg:text-8xl font-black text-[var(--text-primary)] tracking-tighter uppercase leading-[0.85]">
              ...
              <button 
                ...
                className="h-16 px-10 rounded-[2rem] bg-[var(--bg-primary)] border border-[var(--glass-border)] text-[var(--text-primary)] font-black text-xs uppercase tracking-[0.3em] flex items-center gap-4 transition-all hover:bg-white/5 disabled:opacity-50"
                disabled={refreshing}
              >
              ...
              >
                 <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                       <h3 className="text-lg font-black text-[var(--text-primary)] uppercase tracking-tight">{node.name}</h3>
                       <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] opacity-40">{node.region}</p>
                    </div>
                 </div>

                 <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10">
                    <div className="space-y-2">
                       <p className="text-[8px] font-black uppercase tracking-[0.4em] text-[var(--text-secondary)] opacity-40">Operational Status</p>
                       <div className="flex items-center gap-2 text-emerald-500">
                          <CheckCircle className="size-4" />
                          <span className="text-[11px] font-black uppercase tracking-widest leading-none">Operational</span>
                       </div>
                    </div>
                    <div className="h-10 w-px bg-[var(--glass-border)] hidden md:block" />
                    <div className="space-y-2">
                       <p className="text-[8px] font-black uppercase tracking-[0.4em] text-[var(--text-secondary)] opacity-40">System Latency</p>
        ...
        <div className="glass-panel p-10 rounded-[3rem] border border-[var(--glass-border)] bg-gradient-to-br from-[var(--bg-primary)]/60 to-transparent">
           <div className="flex items-center justify-between mb-8">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--text-secondary)]">Historical Availability (Last 90 Days)</h3>
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">99.98% Uptime</span>
           </div>
           <div className="flex gap-[2px] h-12 overflow-hidden items-end">
              {[...Array(90)].map((_, i) => (
                 <div key={i} className="flex-1 bg-emerald-500/20 rounded-full h-full hover:bg-[var(--accent)] transition-all cursor-crosshair group relative" title={`Day ${90-i} Status: Operational`}>
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-black text-white text-[8px] font-bold rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-all">Status: Nominal (100%)</div>
                    <div className="w-full bg-emerald-500/40 h-full rounded-full" />
                 </div>
              ))}
           </div>
           <div className="flex justify-between items-center mt-4">
              <p className="text-[8px] font-black uppercase tracking-widest text-[var(--text-secondary)] opacity-40">90 Days Ago</p>
              <div className="flex-1 h-px bg-[var(--glass-border)] mx-10 opacity-30" />
              <p className="text-[8px] font-black uppercase tracking-widest text-[var(--text-secondary)] opacity-40">Today</p>
           </div>
        </div>
