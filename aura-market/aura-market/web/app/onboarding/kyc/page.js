      ...
      <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex items-center justify-center p-6 md:p-12 relative overflow-hidden">
         ...
         >
            ...
            <div className="w-full md:w-80 bg-[var(--bg-primary)]/40 border-r border-[var(--glass-border)] p-12 flex flex-col gap-12">
               <button onClick={() => router.back()} className="flex items-center gap-2 text-[10px] font-black tracking-widest uppercase opacity-40 hover:opacity-100 transition-opacity">
                  <ArrowLeft className="size-4" /> Go Back
               </button>

               <div className="space-y-4">
                  <ShieldCheck className="size-10 text-[var(--accent)]" />
                  <h1 className="text-3xl font-black tracking-tighter uppercase leading-tight">KYC Port Protocol</h1>
                  <p className="text-[10px] font-bold text-[var(--text-secondary)] opacity-60 uppercase tracking-widest">Aura Market Partner Compliance v6.0</p>
               </div>
               ...
               <nav className="space-y-8 mt-12">
                  {steps.map((s, i) => (
                     <div key={s.id} className={`flex items-start gap-4 transition-all ${step >= s.id ? 'opacity-100' : 'opacity-20'}`}>
                        <div className={`size-8 rounded-xl flex items-center justify-center text-[10px] font-black border ${step === s.id ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--glass-border)]'}`}>
                           {step > s.id ? <CheckCircle2 className="size-4" /> : s.id}
                        </div>
                        <div className="space-y-1">
                           <span className="text-[10px] font-black uppercase tracking-widest block">{s.title}</span>
                           <p className="text-[8px] font-bold text-[var(--text-secondary)] uppercase">{s.desc}</p>
                        </div>
                     </div>
                  ))}
               </nav>
               ...
               <div className="mt-auto space-y-4 border-t border-[var(--glass-border)] pt-8">
                  <div className="flex items-center gap-3 text-[9px] font-black tracking-widest uppercase opacity-40">
            ...
            <div className="flex-1 p-12 md:p-20 flex flex-col justify-center max-w-2xl mx-auto">
               <AnimatePresence mode="wait">
                  {step === 1 && (
                     ...
                     >
                        <div className="space-y-2">
                           <h2 className="text-4xl font-black tracking-tight uppercase">Identity Matrix</h2>
                        ...
                        <div className="space-y-6">
                           <div className="space-y-3">
                              <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] ml-2">ID Protocol Type</label>
                              <select className="w-full h-16 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-2xl px-6 text-sm font-bold uppercase tracking-widest outline-none focus:ring-2 focus:ring-[var(--accent)]/30">
                        ...
                        <button
                           onClick={() => setStep(2)}
                           className="w-full py-6 rounded-3xl bg-[var(--text-primary)] text-[var(--bg-primary)] font-black uppercase tracking-[.3em] text-[10px] flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-95 transition-all shadow-xl"
                        >
                  ...
                  {step === 2 && (
                     ...
                     >
                        <div className="space-y-2">
                           <h2 className="text-4xl font-black tracking-tight uppercase">Base Verification</h2>
                        ...
                        <div className="p-12 rounded-[40px] border-2 border-dashed border-[var(--glass-border)] bg-[var(--bg-primary)] shadow-inner flex flex-col items-center justify-center text-center gap-6 group hover:border-[var(--accent)]/30 transition-all cursor-pointer">
                           ...
                           <div className="space-y-1">
                              <span className="text-[10px] font-black uppercase tracking-widest block">Drop Proof Artifact Here</span>
                              <p className="text-[9px] font-bold text-[var(--text-secondary)] uppercase opacity-40">PDF / JPEG / PNG MAX 10MB</p>
                           </div>
                        </div>

                        <div className="flex gap-4">
                           <button onClick={() => setStep(1)} className="flex-1 py-6 rounded-3xl border border-[var(--glass-border)] font-black uppercase tracking-widest text-[9px] hover:bg-[var(--bg-secondary)] transition-all">Previous</button>
                           <button onClick={() => setStep(3)} className="flex-[2] py-6 rounded-3xl bg-[var(--text-primary)] text-[var(--bg-primary)] font-black uppercase tracking-[.3em] text-[10px] shadow-xl hover:scale-[1.02] transition-all">Continue Sync</button>
                        </div>
                     </motion.div>
                  )}
                  ...
                  {step === 3 && (
                     ...
                     >
                        ...
                        <div className="space-y-4">
                           <h2 className="text-4xl font-black tracking-tight uppercase">Biometric Initiation</h2>
                        ...
                        <button
                           ...
                           className="w-full py-8 rounded-[32px] bg-[var(--accent)] text-white font-black uppercase tracking-[.4em] text-[11px] flex items-center justify-center gap-4 hover:scale-[1.02] shadow-[0_20px_60px_rgba(242,13,242,0.3)] transition-all disabled:opacity-50"
                        >
